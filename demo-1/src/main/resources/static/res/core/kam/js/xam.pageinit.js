...
function pageInit(isPopup){
    ...
    $('#btn_delete').off("click").on("click", function(){
    	...
    	$.each(sel_obj, function(idx, obj) {
			restore_size += parseInt($(this).find(".finfo .real-size").text());
		});
		var result = false;
    	...
    	$.each(sel_obj, function(idx, obj) {
			...
			if (_XAM.args.isEdit && file.status == 'Complete') {
				if ($(this).hasClass('remove')) {
					...
					var tsize = restore_size + $("#fileupload").data('_XAM-fileupload').normal_file_size_check();
					var total_limit = parseInt(_XAM.env.attach_limit_size) * 1024 * 1024;
					if (tsize > total_limit){
						result = true;
						$(this).find(".checkbox input[type=checkbox]").prop('checked', false)
		...
		if (!result){
			...
			$("#fileupload").data('_XAM-fileupload').draw_file_size();
			return false;
    	} else {
			var disp_size;
			...
			alert(_XAM.xamlang.propStr("kam_limit_size", disp_size));
    ...
    $(document).ready(function () {
    	var _h = (isPopup?'192px':'160px');
    	$("#file_list table").freezeHeader({ 'height': _h, 'resize': '' });
    });
}

function callPreview(file){
	var eext = file[0].filename.lastIndexOf(".");
   	var fileext  = file[0].filename.substr(parseInt(eext)+1).toLowerCase();
	
	// 미리보기 가능한 파일 확인
	if (fileext == "doc" | fileext == "docx" | fileext == "txt" | fileext == "log" | fileext == "html" | fileext == "htm" | fileext == "xls" | fileext == "xlsx" | fileext == "ppt" | fileext == "pptx" | fileext == "pdf" | fileext == "xps" | fileext == "bmp" | fileext == "gif" | fileext == "tiff"  | fileext == "tif" | fileext == "jpg" | fileext == "jpeg" | fileext == "png" | fileext == "hwp") {
	} else {
		alert(_XAM.xamlang.propStr("kam_preview_no"));
		return false;
	}

	// 파일명으로 해당 파일만 추출
	var file_url = file[0].fileDownUrl;
	var v_file_url = file_url.split("/");
	var t_mail_path = v_file_url[1] + "/" + v_file_url[2] + "/" + v_file_url[3];
	var t_mail_unid = v_file_url[5];
	var t_mail_file_name = decodeURIComponent(v_file_url[7]);

	if (t_mail_file_name.indexOf('&') > -1) {
		t_mail_file_name = t_mail_file_name.replace(/&/g, "#.#");
	}
	if (parent.current_mail_server == undefined) {
		var t_mail_server = v_file_url[1] + "a";
	} else {
		var t_mail_server = parent.current_mail_server;
	}
	
	var junction = v_file_url[1];
	var t_process_url = "http://" + window.location.host + "/ngw/core/lib.nsf";
	var filenumber = file[0].order;
	try { 
	...
	// 추출한 파일 경로 리턴
	var appURL = 'https://'+location.host+'/'+junction+'/mail/resource/lib.nsf/agt_comn_convertToIamge?openAgent&as='+t_mail_server+'&ap='+t_mail_path+'&unid='+t_mail_unid+'&fn='+encodeURIComponent(t_mail_file_name)+'&fno='+filenumber+'&eiplan='+eip_lang;
	$.ajax({
		type: 'GET'
		,url: appURL
		,timeout: 600000
		...
		,success: function(data) {
			if (data=='nofile') {
				alert(_XAM.xamlang.propStr('kam_preview_none'));
			} else {
				setTimeout(function(){
					openSynapViewer(data, t_mail_unid,eip_lang);
				}, 500);
			}
		}
		,error: function(x) {
			alert(_XAM.xamlang.propStr('kam_preview_error'));
		}
	});	
}

function openSynapViewer(filePath, unid,eip_lang){
	var fid = unid;	 // UNID 입력
	var gl_synapurl = 'https://dsw.daesang.com/SynapDocViewServer';
	...
	$.ajax({
		...
		success: function (data) {
			setTimeout(function(){
				window.open(gl_synapurl + "/" + data.viewUrlPath);
			}, 1000);		
		},
					...
					error: function (error) {
						if (error.status == 500) {
							$.ajax({
								...
								error: function (error) {
									alert(_XAM.xamlang.propStr('kam_preview_busy_server'));
								}
							});
						} else {
							alert(_XAM.xamlang.propStr('kam_preview_error'));
