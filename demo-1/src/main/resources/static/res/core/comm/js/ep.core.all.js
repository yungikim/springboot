 ...
 */
 (function($) {
	...
	var _ui = $ep.ui = {
		...
		//		,content : {
		//			url : root_path + "/page/user_profile.jsp?userid=" + _id	//$ep._CONST.PATH.ORG + "/user_by_notesid_profile/" + encodeURI(_id) /* encodeURIComponent(_id)*/ + "?opendocument"
		//		}
		//		
		//	},opt));
			...
			var url = root_path + "/page/user_profile.jsp?userid=" + _id;
			window.open(url, 'user_profile', 'top='+_t+',left='+_l+',width='+_w+',height='+_h);
		}
	//	,isShowOrg : null
		,isShowOrg : "T"
		...
		,activeXMethods : function() {
			...
			return  {
				...
				,submit 				: function() {
					...
					for(var x=0;x < cnt;x++) {	
						console.log("%%File."+x);
						this.api("AddPostData","file","%%File."+x,"");
					}
	...
	var _organ = {
		...
		,_convertViewEntry : function(_d,opt) { /* display options 빼고 모두 적용. */
			...
			_r = [];
			var eip_lang = parent.user_eip_lang;
				...
				if (__r["type"] == "PC") {
					...
					if (__r["type"] == "PG") {
						...
						if ($ep.lang() == "ko") {
							__r["title"] = _compItem ? _compItem.title ? _util.patternCompletion(_compItem.title, __r) : __r["name"] : __r["name"];
							__r["tooltip"] = _compItem ? _compItem.tooltip ? _util.patternCompletion(_compItem.tooltip, __r) : __r["title"] : __r["title"];						
						// 한국어가 아닌 언어이고 사용자 언어와 같으면 로컬 언어
						} else if ($ep.lang() == __r["lang"]) {
							__r["title"] = __r["ocompany"] ? __r["ocompany"] : __r["ecompany"];
							__r["tooltip"] = __r["title"];						
						// 다르면 영어
						} else {
							__r["title"] = __r["ecompany"] ? __r["ecompany"] : __r["company"];
							__r["tooltip"] = __r["title"];							
						}						
					}
				}
		...
		,_convertOrgData : function(_entry){
			var _data = $.isArray(_entry) ? _entry[0] : _entry;
			var _d = new Object();

			_d.type = _data.type ? _data.type : "";
			_d.company = _data.cp ? _data.cp : "";
			_d.notesid = _data.emp ? _data.emp : "";
			_d.companycode = _data.cpc ? _data.cpc : "";
			_d.dispgrade = _data.dg ? _data.dg : "";
			_d.orgname = _data.dp ? _data.dp : "";
			_d.code = _data.dpc ? _data.dpc : (_data.cpc ? _data.cpc : "");
			_d.orgcode = _data.dpc ? _data.dpc : "";
			_d.duty = _data.du ? _data.du : "";
			_d.ecompany = _data.ecp ? _data.ecp : "";
			_d.eorgname = _data.edp ? _data.edp : "";
			_d.eduty = _data.edu ? _data.edu : "";
			_d.epost = _data.ejt ? _data.ejt : "";
			_d.lang = _data.el ? _data.el : "";
			_d.mailfile = _data.mf ? _data.mf : "";
			_d.email = _data.em ? _data.em : "";
			_d.empno = _data.emp ? _data.emp : "";
			_d.orgnumber = _data.emp ? _data.emp : "";
			_d.ename = _data.enm ? _data.enm : "";
			_d.userid = _data.id ? _data.id : "";
			_d.phonenumber = _data.iptno ? _data.iptno : "";
			_d.post = _data.jt ? _data.jt : "";
			_d.postcode = _data.jtc ? _data.jtc : "";
			_d.cellphonenumber = _data.mp ? _data.mp : "";
			_d.name = _data.nm ? _data.nm :"";
			_d.ocompany = _data.ocp ? _data.ocp :"";
			_d.oorgname = _data.odp ? _data.odp : "";
			_d.oduty = _data.odu ? _data.odu : "";
			_d.opost = _data.ojt ? _data.ojt : "";
			_d.oname = _data.onm ? _data.onm : "";
			_d.officephonenumber = _data.op ? _data.op : "";
			_d.subdept = _data.subdept ? _data.subdept : "";
			_d.authid = _data.auth_id ? _data.auth_id.replace(/\,/g, "/") : "";
				
			_d.fulldeptcode = "";
			_d.fulldeptname = "";
			
			if (_d.type == "D"){
				_d.name = _data.dp ? _data.dp : (_data.cp ? _data.cp : "");
				_d.ename = _data.edp ? _data.edp : (_data.ecp ? _data.ecp : "");
				_d.oname = _data.odp ? _data.odp : (_data.ocp ? _data.ocp : "");
			}
		...
		*/
		,_initTreeNodes : function(_key,_callback) {
			...
			var _code = _uri.getParam("category");
			var _pd = {code : _code};
			var _url = root_path + "/organ_tree_all.km";
			//_util.ajax({type:"get",async : true, url : _uri.url})
			_util.ajax({type:"post",async : true, data : JSON.stringify(_pd),url : _url})
			.done(function(_data,txt,xhr) {
				if(!_self.element) {return;}
				var _d = [];
				if (_data.company){
					$.each(_data.company, function(idx, val){
						val.type = "D";
						_d.push(val);
					});
				}
				if (_data.user){
					$.each(_data.user, function(idx, val){
						val.type = "P";
						_d.push(val);
					});
				}				
				if (_data.sub_dept){
					$.each(_data.sub_dept, function(idx, val){
						val.type = "D";
						if (val.dpc != "000000"){
							_d.push(val);
						}
				...
				_result = _organ._filter.call(_self,_d);
...
/*		,_restfilter : function(_d) {
		...
		,_parse_org : function(_entry) {
			...
			if(!_entry) {return _r;}
			_r = _organ._convertOrgData(_entry);//_organ._convertEntry(_entry);
			switch(_r["type"]) {
			case "D":   /* 부서/팀 */
			...
			case "P":  /* 직원 */
				...
				_r["oduty"] = _r["oduty"] && _r["oduty"].replace(/\(.*\)/g,"");
				_r["opost"] = _r["opost"] && _r["opost"].replace(/\(.*\)/g,"");
				break;
			}
			...
			// 한국어가 아닌 언어이고 사용자 언어와 같으면 로컬 언어
			} else if ($ep.lang() == _r["lang"]) {
				// _r["title"] = _r["oname"] ? _r["oname"] : _r["ename"];		
				_r["title"] = _compItem ? _compItem.title ? _util.patternCompletion(_compItem.title, _r) : _r["oname"] : _r["oname"];	
			// 다르면 영어
			} else {
				// _r["title"] = _r["ename"] ? _r["ename"] : _r["name"];	
				_r["title"] = _compItem ? _compItem.title ? _util.patternCompletion(_compItem.title, _r) : _r["ename"] : _r["ename"];	
			}
				...
				"single" : {
						...
						,tabs : {
							organ : {
								...
								,url : root_path + "/res/core/comm/html/organ/organ.tab.organtree.html"
					   ...
					   ,{ id : "orgname" , label : "{$CORE.ORGAN.GRID.TEAM}", width : "*" 
						  ,expression : function(td,colset,data) {
							...
							if ($ep.lang() == "ko") {
								...
								if (data.type == "D") {
									_orgname = data.ocompany;
								} else {
									_orgname = data.oorgname;
								}
							} else {
								if (data.type == "D") {
									_orgname = data.ecompany;
								} else {
									_orgname = data.eorgname;
								}
							}
							return _orgname; 
						  }
		...
		,_organPubGroup : function(__opt) {
			return {
				...
				,url : root_path + "/res/core/comm/html/organ/organ.tab.pubgroup.html"
				,initPage : false  /*초기화 작업을 URL에서 처리.*/
				,tree : {
		...
		,_organPerGroup : function(__opt) {
			return {
				...
				,url : root_path + "/res/core/comm/html/organ/organ.tab.pergroup.html"
				,initPage : false
				,tree : {
					root : "pergroup"
					,server : __opt.server || "" //대표서버
		...
		,_organPerAddr : function(__opt) { /*type = PC */
			return {
				...
				,url : root_path + "/res/core/comm/html/organ/organ.tab.peraddr.html"
				,initPage : false
				,grid : {
					keycode : "key"
					,isduplicate :false
				...
				organ : {
					...
					,tree : {
						...
						,itemRender : function(data) {
							...
							//2023.11.15 개인주소록에서 부서 선택 안되게 막기
							if(data.type == "D" && location.href.indexOf("addr_list_org") > -1) {
								data["unselectable"] = true;
								data["hideCheckbox"] = true;
							}
						}
						...
						,item : function(data) {
							...
							// 한국어이면 무조건 한국어
							if ($ep.lang() == "ko") {
								__title = "{name}" + (data.post?' - {post}':'') + (data.duty?' [&#91;{duty}&#93;]':'');
							// 한국어가 아닌 언어이고 사용자 언어와 같으면 로컬 언어
							} else if ($ep.lang() == data.lang) {
								__title = "{oname}" + (data.opost?' - {opost}':'') + (data.oduty?' [&#91;{oduty}&#93;]':'');
							// 다르면 영어
							} else {
								__title = "{ename}" + (data.epost?' - {epost}':'') + (data.eduty?' [&#91;{eduty}&#93;]':'');
							}
							var _tooltip = "{email}";
							return { 
								title : __title
						...
						organ : {
							...
							,tree : {
								...
								,item : __opt.treeItemExpression || function(data) {
									...
									// 한국어이면 무조건 한국어
									if ($ep.lang() == "ko") {
										__title = "{name} " + (data.duty?' [&#91;{duty}&#93;]':'');
									// 한국어가 아닌 언어이고 사용자 언어와 같으면 로컬 언어
									} else if ($ep.lang() == data.lang) {
										__title = "{oname} " + (data.oduty?' [&#91;{oduty}&#93;]':'');
									// 다르면 영어
									} else {
										__title = "{ename} " + (data.eduty?' [&#91;{eduty}&#93;]':'');
									}
									...
									/*
									return { 
										title : data.lang == $ep.lang() ? "{name} " + (data.duty?' [&#91;{duty}&#93;]':'') : data.ename ? "{ename} "  + (data.eduty?' [&#91;{eduty}&#93;]':'') : "{name} " + (data.duty?' [&#91;{duty}&#93;]':'')
										,tooltip : _tooltip
									};
									*/
									/*
...
/*		,getActiveTree : function() {	return $(this._activetabs["panel"]).data("organ_tree");	}
	...
	$ep.ui.organ.tree.method = $ep.ui.organ.tree.prototype = {
		...
		,initTree : function() {
			...
			_self.api.tree = $ep.ui.tree($(_self.element),{
				...
				,dblclick : function(e,data) { 
					...
					_dbl.clear();
					if(typeof _opt.dblclick === "function") {
						if(_opt.dblclick.call(_self,this,data) === false)
						{
							return;
						};
					}
					if(_opt.checkbox && data.node.isExpanded()) {
						this.childrenSelectedAll(data.node);
						data.node.setSelected(false);
					} 
					if(data.node.hasChildren() || data.node.isLazy()) {
						if(!(_opt.checkbox && data.node.isExpanded())){	
		...
		/*	, _uri = $.CURI(
				...
				, {
					search : query, 
					ps : $ep._CONST.DATASERVICEMAXVIEWENTRIES,sortcolumn : 
						$ep.lang() != "en" ? "_name" : "_ename" ,
						sortorder : "ascending"
				}
			).encode();*/
			, _uri = $.CURI(root_path + "/search_user_multi.km").encode();
			_util.ajax({
				type:"post"
			...
			.done(function(_data,txt,xhr) {
				var _data = (_data[0].length == 0 ? [{}] : _data);
				var _d = [];
				$.each(_data[0], function(idx, val){
					val.type = "P";
					_d.push(val);
				});
				var _result = _organ._gridfilter.call(_self,_d);
	...
	$ep.ui.organ.mailtree.method = $ep.ui.organ.mailtree.prototype = $.extend(true,{},$ep.ui.tree.method,{
		...
		,create : function(){
			...
			_self.options = $.extend(true,{
				...
				,checkbox : true
				,selectMode : 3
				,click : function(e,ctr,data) { 
					if (ctr.targetType == "title") {
						var _select_count = 0;
						
						if(ctr.node.isFolder()) {return true;}
						if(!ctr.node.parent) {return true;}
						if (ctr.node.selected == true) {
							ctr.node.selected = false;
							$(ctr.node.span).removeClass("fancytree-focused fancytree-selected");
						} else {
							ctr.node.selected = true;
							$(ctr.node.span).addClass("fancytree-focused fancytree-selected");
						}
						
						var _parent = ctr.node.getParent();
						if(_parent) {
							_self.options.renderNode.call(_parent.tree,{type : "renderNode"},_parent.tree._makeHookContext(_parent,null));
							
							var _count =  _parent.children ? _parent.children.length : 0;
							
							for(var c=0; _parent.children.length > c; c++) {
								if (_parent.children[c].selected == true) {
									_select_count += 1;
								}
							}
							
							if (_select_count > 0 ) {
								_parent.partsel = true;
								$(_parent.span).addClass(_parent.tree.options._classNames.partsel);
							} else {
								_parent.partsel = false;
								$(_parent.span).removeClass(_parent.tree.options._classNames.partsel);
							}
							
							if (_select_count == _count ) {
								$(_parent.span).addClass(_parent.tree.options._classNames.selected);
							} else {
								$(_parent.span).removeClass(_parent.tree.options._classNames.selected);
							}
