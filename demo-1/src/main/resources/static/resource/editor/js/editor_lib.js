...
(function() {
  ...
  if(!Array.prototype.map) Array.prototype.map = function(f, thisArg) {
		for (var b = [], i = 0, n = this.length; i < n; ++i) {
			b[i] = (thisArg?f.call(thisArg, this[i], i, this):f(this[i], i, this));
		}
		return b;
	};
	/**
...
goog.dom.SavedCaretRange.prototype.removeCarets = function(opt_range) {
  if($tx.msie_docmode <= 8){
    // KEDITOR Caret이 중복됨.(내용이 Caret안쪽으로 들어감.)
    goog.dom.flattenElement(this.getCaret(_TRUE));
    goog.dom.flattenElement(this.getCaret(_FALSE));
  }else{
...
goog.dom.browserrange.IeRange.insertNode_ = function(clone, node,
  ...
  if (node.nodeType != goog.dom.NodeType.ELEMENT) {
    ...
    node = opt_domHelper.createDom(goog.dom.TagName.SPAN, _NULL, node); // P tag 안쪽으로 DIV가 들어갈 수 없음. SPAN으로 대체
  }
...
(function(){
  ...
  var _dreg = /(\S*)\.(\S+)/;
	var filters = {
		...
		'.': function(cnxt, expr){
			if ((m = _dreg.exec(expr)) !== _NULL) {
				var tag = ((m[1] === "") ? "*" : m[1]);
				var klass = m[2];
				if ((els = cnxt.getElementsByTagName(tag)).length > 0) {
					var results = [];
          var _clsReg = (new RegExp("(^| )" + klass + "($| )"));
					for (var i=0; i<els.length; i++) {
						var el = els[i];
						if ( _clsReg.test(el.className)) {
							results.push(el);
						}
					}
					return results;
	...
	var match = function(cnxt, expr) {
        ...
        var fltr;
        if ((f = _matchreg.exec(expr)) !== _NULL) {
            if (filters[f[1]]) {
                fltr = f[1];
            }
        }
...
(function(){
  ...
  var _dreg = /(\S*)\.(\S+)/;
	...
	var check = function(cnxt, expr) {
        var fltr;
        if ((f = _checkreg.exec(expr)) !== _NULL) {
            if (filters[f[1]]) {
                fltr = f[1];
            }
        }
