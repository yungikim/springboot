...
gAdminGpt.prototype = {
	"init" : function(){
		...
		gap.curLang = gap.getCookie("language");
		//this.showBlock(true);
		...
		// 첫 번째 전체 추가함
		this.addFolder('all', '전체');
		this.eventBind();
		$('#folder_list li:first-child').addClass('on');
		


		var defs = [];
		defs.push(this.drawFolderList());
		defs.push(this.drawUploadList(1, true));
		$.when.apply($, defs).then(function(){
			setTimeout(function(){				
				_self.hideBlock(true);				
			}, 500);
		});
	...
	"showCreateFolderLayer" : function(){
		...
		var html = 
			...
			'				<span>폴더명</span>' +
			'				<div>' +
			'					<input type="text" id="folder_name" autocomplete="off" placeholder="폴더명을 입력하세요.">' +
			'				</div>' +
			'			</div>' +
			'		</div>' +
			'		<div class="btn-wr">' +
	...
	"showRemoveFolderLayer" : function(folder_code, folder_name){
		...
		var html = 
			...
			'		<div class="layer-cont">' + 
			'			<div class="inner-wr">' +
			'				<div style="padding:10px 0;">"' + folder_name+ '" 폴더를 삭제 하시겠습니까?</div>' +
			'				<span style="color:#ff0000;font-size:13px">중요: 폴더에 포함된 모든 파일이 함께 삭제됩니다</span>' +
			'			</div>' +
			'		</div>' +
			'		<div class="btn-wr">' +
	...
	"showAddFileLayer" : function(info){
		...
		var html = 
			...
			'				<div class="tab-wrap">' +
			'					<div class="upload-tab type-1 on" id="tab_share" data-type="1">파일 콘텐츠</div>' +
			'					<div class="upload-tab type-2" id="tab_api" data-type="2">텍스트 콘텐츠</div>' +
			'					<div class="upload-tab type-3" id="tab_url" data-type="3">Web URL</div>' +
			'				</div>' +
			...
			'					</div>' +
			'					<div class="each">' +
			'						<div class="menu-title">파일</div>' +
			'						<input class="upload-file-name" autocomplete="off" placeholder=".docx, .pptx, .pdf, .txt만 업로드 가능" readonly>' +
			'						<label for="upload_file" class="upload-file-btn">파일찾기</label>' +
		...
		$('#upload_file').on('click', function(){
			if ($layer.hasClass('edit')) {
				mobiscroll.toast({message:'파일은 변경할 수 없습니다', color:'danger'});
				return false;
			}
		});
		...
		$layer.find('.confirm').on('click', function(){
			...
			if (_subj == '') {
				mobiscroll.toast({message:'제목을 입력하세요', color:'danger'});
			...
			if (is_edit) {
				...
				var _folder_code = $('#folder_list li.on').data('key');
				if (_folder_code == 'all') {
					_folder_code = '';
				}
			}
			...
			if (_type == '1') {
				...
				if (!_file) {
					mobiscroll.toast({message:'파일을 선택하세요', color:'danger'});
					$('#upload_file').click();
					return;
				}
				...
				if (_cont == '') {
					mobiscroll.toast({message:'학습할 내용을 입력하세요', color:'danger'});
				...
				if (_url == '') {
					mobiscroll.toast({message:'URL을 입력하세요', color:'danger'});
	...
	"setEditInfo" : function(info){
		...
		$layer.find('.upload-tab').off().on('click', function(){
			...
			mobiscroll.toast({message:'다른 유형으로 변경할 수 없습니다', color:'danger'});
		});
		...
		if (info.type == '1') {
			...
			var data = {key: info.key, category : "admin"};
			$.ajax({
				type: 'POST',
	...
	"fileUpload" : function(fd){
		...
		return $.ajax({
			...
			processData: false,
			success: function(res){
				mobiscroll.toast({message:'파일이 등록되었습니다', color:'info'});
	...
	"apiSave" : function(fd){
		...
		return $.ajax({
			...
			contentType: 'application/json;charset=utf-8',
			success: function(res){
				mobiscroll.toast({message:'등록되었습니다', color:'info'});
				$('.add-file-layer').remove();
				_self.drawUploadList(1, true);
			},
			error: function(){
				mobiscroll.toast({message:'오류가 발생했습니다', color:'danger'});
			}
		});
	...
	"createFolder" : function(){
		...
		if ($.trim(folder_nm) == '') {
			mobiscroll.toast({message:'폴더명을 입력하세요', color:'danger'});
			return false;
		}
		...
		$.ajax({
			...
			contentType : "application/json; charset=utf-8",
			beforeSend: function(){
				gAg.showLoading('Loading');
			},
			data: JSON.stringify(fd),
			success: function(res){
				if (res.result == 'OK') {
					mobiscroll.toast({message:'폴더가 생성되었습니다', color:'info'});
		...
		return $.ajax({
			...
			success: function(res){
				if (res.result == 'OK') {
					...
					$('#folder_list').find('li[data-key="' + key + '"]').remove();					
				}
				_self.hideLoading();
			}
		});
	...
	"drawUploadList" : function(page_no, is_init){
		...
		return $.ajax({
			...
			success: function(res){
				...
				// 제목 삽입
				$('#filelist_title').html('"<span style="color:#0000ff;">' + name + '</span>" 파일 리스트');
				
				if (res[0].totalcount == 0) {
					$('#file_list').append('<li class="empty-file">표시할 내용이 없습니다</li>');
				}
	...
	"addFile" : function(info){
		...
		var html = 
			...
			'			<button class="ico ico-down" title="다운로드"></button>' +
			'			<button class="ico ico-del" title="삭제"></button>' +
			'		</div>' +
			'	</div>' +
			'</li>';
	...
	"showRemoveFileLayer" : function(filekey, filename){
		...
		var html = 
			...
			'				<span style="color:#ff0000;font-size:13px">선택하신 파일을 삭제 하시겠습니까?</span>' +
			'			</div>' +
			'		</div>' +
			'		<div class="btn-wr">' +
			'			<button class="confirm">확인</button>' +
		...
		// 확인 버튼
		$layer.find('.confirm').on('click', function(){
			_self.showLoading('삭제 중...');
			_self.removeFile(filekey).then(function(){
				$layer.find('.btn-close').click();
				mobiscroll.toast({message:'파일이 삭제되었습니다', color:'info'});
				_self.drawUploadList();
			});
		});
	...
	"drawQuestionList" : function(page_no, is_init){
		...
		return $.ajax({
			type: 'POST',
			url: this.gptserver_fast + 'folder/question_list',
			contentType : "application/json; charset=utf-8",
			beforeSend: function(){
				_self.req_list = setTimeout(function(){
					_self.showLoading('Loading');
	...
	"drawComplainList" : function(page_no, is_init){
		...
		return $.ajax({
			type: 'POST',
			url: this.gptserver_fast + 'folder/complain_list',
			contentType : "application/json; charset=utf-8",
			beforeSend: function(){
				_self.req_list = setTimeout(function(){
					_self.showLoading('Loading');
		...
		return $.ajax({
			...
			success: function(__res){
				...
				for (var i = 0; i < _list.length; i++){
					var _info = _list[i];						
					var _key = _info.code;
					var _icon_src = root_path + "/plugin_download_gpt.do?code=" + _key + '&ver=' + _info.last_update;
	...
	"showPluginUploadLayer" : function(info){
		...
		if (is_edit) {
			...
			// 아이콘
			var icon_src = root_path + "/plugin_download_gpt.do?code=" + info.code + '&ver=' + info.last_update;
	...
	"menu_upload_event" : function(is_edit){
		...
		window.myDropzone_menuico = new Dropzone("#" + selectid, { // Make the whole body a dropzone
			url: root_path + "/FileControl_plugin.do",
	...
	"callRealtimeTraining" : function(){
		...
		$.ajax({
			type: 'POST',
			url: this.gptserver_fast + 'run_script_admin',
			contentType : "application/json; charset=utf-8",
			beforeSend: function(){
				_self.showLoading('요청 중');
			},
			success: function(res){
				if (res.result == 'OK') {
					setTimeout(function(){
	...
	"initialize_page" : function(){
		var alldocuments = gAg.total_cnt;
		if (alldocuments % gAg.per_page > 0 & alldocuments % gAg.per_page < gAg.per_page/2 ){
			gAg.all_page = Number(Math.round(alldocuments/gAg.per_page)) + 1
		}else{
			gAg.all_page = Number(Math.round(alldocuments/gAg.per_page))
		}	

		if (gAg.start_page % gAg.per_page > 0 & gAg.start_page % gAg.per_page < gAg.per_page/2 ){
			gAg.cur_page = Number(Math.round(gAg.start_page/gAg.per_page)) + 1
		}else{
			gAg.cur_page = Number(Math.round(gAg.start_page/gAg.per_page))
		}
	...
	"initialize_navigator" : function(){
		...
		if (gAg.total_page_count == 0){
			gAg.total_page_count = 1;
		}
		...
		if (alldocuments != 0) {
			...
			if (gAg.total_page_count % 10 > 0 & gAg.total_page_count % 10 < 5 ){
				var all_frame = Number(Math.round(gAg.total_page_count / 10)) + 1
			}else{
				var all_frame = Number(Math.round(gAg.total_page_count / 10))	
			}

			if (gAg.cur_page % 10 > 0 & gAg.cur_page % 10 < 5 ){
				var c_frame = Number(Math.round(gAg.cur_page / 10)) + 1
			}else{
				var c_frame = Number(Math.round(gAg.cur_page / 10))
			}
			...
			if (c_frame == 1){
				...
				nav[0] = '<li title="이전" class="p-prev" onclick="gAg.goto_page(' + ((((c_frame-1) * 10) - 1)*gAg.per_page + 1) + ',' + ((c_frame - 1) * 10) + ');"><span>&lt;</span></li>';
			}			
			...
			for (var i = start_page; i < start_page + 10; i++){
				if (i == gAg.cur_page){
					...
					if (i == '1'){
						nav[pIndex] = '<li onclick="gAg.goto_page(' + (((i-1) * gAg.per_page) + 1 ) + ', ' + i + ', this)">' + i + '</li>';
					}else{
						if (i % 10 == '1' ){
							nav[pIndex] = '<li onclick="gAg.goto_page(' + (((i-1) * gAg.per_page) + 1 ) + ', ' + i + ', this)">' + i + '</li>';
						}else{
			...
			if (c_frame < all_frame){
				nav[nav.length] = '<li title="다음" class="p-next" onclick="gAg.goto_page(' + ((c_frame * gAg.per_page * 10) + 1) + ',' + ((c_frame * 10) + 1) + ');"><span>&gt;</span></li>';
			}
			var nav_html = '';

			if (c_frame != 1 ){
				nav_html = '<li title="처음" class="p-first" onclick="gAg.goto_page(1,1);"><span>&lt;&lt;</span></li>';
			}		    
			...
			if (c_frame < all_frame){
				nav_html += '<li title="마지막" class="p-last" onclick="gAg.goto_page(' + ((gAg.all_page - 1) * gAg.per_page + 1) + ',' + gAg.all_page + ')"><span>&gt;&gt;</span></li>';
			}
			
			if (gAg.page_type == "type_4"){
	...
	"goto_page" : function(idx, page_num){
		...
		if (gAg.total_cnt < idx) {
			gAg.start_page = idx - 10;
			if ( gAg.start_page < 1 ) {
				return;
			}
		}else{
		...
		if (gAg.page_type == "type_1"){
			gAg.drawUploadList(page_num);
		} else if (gAg.page_type == "type_2"){
			gAg.drawQuestionList(page_num);
		} else if (gAg.page_type == "type_3"){
			gAg.drawComplainList(page_num);
		} else if (gAg.page_type == "type_4"){
			gAg.drawPluginList(page_num);
		}
