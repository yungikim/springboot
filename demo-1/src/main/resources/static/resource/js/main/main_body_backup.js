...
$('body').off('click.hidemention').on('click.hidemention', function(){
	if (typeof(gBody3) != 'undefined' && gBody3.cur_opt != '') {
...
function gBodyFN(){
	...
	this.dropzone_popup = "";
	this.clipbord_file = "";	
	this.image_max_upload_size = 40;   // 20M  (21000000)
	this.file_max_upload_size = 40;    //40M (53000000)	
...
gBodyFN.prototype = {
	...
	"init" : function(opt){	
		...
		// LNB메뉴 및 더보기 메뉴 정보 가져오기
		if (!this.lnb_menu_load) {
			this.load_person_app();			
		}		
		if (call_key == ""){
		...
		//채팅방의 unread 체크는 websocket에서 처리한다.
		//gBody.unread_chat_count_check();		
		//휴가일정 관리자 인가를 체크하는 함수
		gap.cal_manger_check();	
		

		// 권한이 있는 사용자면 왼쪽 메뉴 보여줌
		if (role == "T"){
			$(".left-menu").show();
		}else{
			$(".wrap").css("padding-left", "0");
			$("#left_main").css("left", "0");
...
//				gBody.set_etc_info(gap.etc_info);						
		...
		if (gap.param == "todo" || gap.param == "drive"){
			//멤버리스트에서 대화방 생성을 바로 할 수 있기 때문에 호출한다.
			//채팅방 리스트 그린다.
//			_wsocket.load_buddy_list();	
//			gBody.set_etc_info(gap.etc_info);
			_wsocket.load_chatroom_list();			
		...
		//	}			
			...
			$("#tab3").hide();
			$("#tab4").hide();			
			$("#show_video_layer").hide();
			$("#video_add_member").hide();
		}		
		//채널을 메인으로 이동하면서 여기서는 주석처리하고 gBody3에서 channel_list.km 마지막에 show해준다.		
	//	if ($("#left_channel").css("display") != "none"){
	//		$("#left_main").show();
	//		$("#main_body").show();
	//	}
				
		if (call_key == ""){			
			gBody3.cur_window = gap.param;			
			if (gap.param == "channel" || gap.param == "home"){			
			}else{
				$("#left_main").show();
				$("#main_body").show();
			}	
...
//				gBody.set_etc_info(gap.etc_info);
				...
				_wsocket.load_chatroom_list_only() //다중채팅방의 경우 기존에 동일한 사용자끼를 방이 있는지 체크하기 위해서 조직도를 로딩할때 채팅방 정보를 업데이트해준다.
				gOrg.showMainOrg();
			}			
			// 회의 새로 고침하는 경우
			if (gap.param == "meeting") {
//				_wsocket.load_buddy_list();	
//				gBody.set_etc_info(gap.etc_info);
...
//				gBody.set_etc_info(gap.etc_info);
	...
	//	gap.draw_qtip_left("#show_memo_layer", gap.lang.sendNotificatioin);
		...
		$("#btn_main_popup").off().on("click", function(e){
			gHome.show_today_popup();
			
			//로그 남기기
			gap.write_log_box("today","오늘할일","menu", "pc");
		});
		
		$("#preview_manual .manual-menu li").on("click", function(){		
			
			$("#preview_manual .manual-menu li").removeClass("on");
			$(this).addClass("on");
			
			var lang = gap.curLang;
			if (gap.curLang != "ko"){
				lang = "en";
			}
			var bun = $(this).attr("id") + "_" + lang;		
			gBody.play_video_manual(bun + ".mp4");			
			return false;
		});
		
		$("#manual_close_btn").off().on("click", function(){
...
//				return false;
		...
		$("#nav_left_menu .tabs .tab").on("click", function(event){
			...
			//	gap.show_content("main");				
				...
				gBody.cur_tab = "tab2";
				gBody.left_height_control();				
				$("#left_roomlist").css("height", "calc(100% - 190px)");				
				 //브라우저 타이틀을 설정한다.
			...
			//	$("#channel_main").hide();
				...
				$("#left_mail_list").css("height", "calc(100% - 5px)");				
				gBody.cur_tab = "tab4";
				gBody.left_height_control();
			}			
		...
		$("#open_emoticon_backup").on("click", function(){
			//채팅창에서 개인 설정 선택한 경우			
			var html = "";
			html += "<div class='layer-emoticon' >";
			html += "	<h2>"+gap.lang.emoti+"</h2>";					
			html += "	<button class='ico btn-article-close'>닫기</button>";
			html += "	<ul class='list-emoticon' id='emoti_dis' style='overflow:hidden'>";			
			html += "		<li><img src='./emoticons/rio.gif' data='rio.gif'></li>";
			html += "		<li><img src='./emoticons/lala.gif' data='lala.gif'></li>";					
			for (var k = 1; k < 41; k++){
				html += "		<li><img src='./emoticons/"+k+".gif' data='"+k+".gif'></li>";
			}			
			html += "	</ul>";
			...
			$("#open_emoticon").qtip({
				...
				events : {
					show : function(event, api){		
						...
						$("#emoti_dis li").on("click", function(e){
							var fname = $(this).find("img").attr("data");
							gBody.send_emoticon_msg(fname);
							return false;							
						});
	...
	"add_chatroom" : function(){
		...
		$("#input_chat_user").keydown(function(evt){
			if (evt.keyCode == 13){
				...
				gBody2.channel_user_search("T", $("#input_chat_user").val());
			}
		})	
		...
		if (type == "file"){					
			...
			if ( (bol) && (gBody.last_enter_type != "ogtag" && gBody.last_enter_type != "emoticon")){
				...
				if ($.isArray(filename) && filename.length > 1){
					...
					if (init == "D"){
						html += "				<div class='img-thumb'><img src='"+downloadurl+"' onerror='this.src=\"img/thm_img.png\"' alt=''></div>";
					}else{
						html += "				<div class='img-thumb'><img src='"+previewurl+"' onerror='this.src=\"img/thm_img.png\"' alt=''></div>";
					}				
				...
				if ($.isArray(filename)){
					...
					html += "			<div class='img-content' data='"+filename+"' data2='"+downloadurl+"'  data3='"+filesize+"'>";				
					if (init == "D"){
						html += "				<div class='img-thumb'><img src='"+downloadurl+"' onerror='this.src=\"img/thm_img.png\"' id='pimg_"+msgid+"' alt=''></div>";
					}else{
						html += "				<div class='img-thumb'><img src='"+previewurl+"' onerror='this.src=\"img/thm_img.png\"' alt=''></div>";
					}				
		...
		if (reply_info != ""){	
			...
			if (reply_info.ex.notice){
				...
				if (rpinfo.ty == "3"){
					...
					rhtml += "<span><img style='width:40px; margin-right:5px' src='./emoticons/" +rpinfo.msg+ "'></span>";				
...
//		아래 소스를 적용하면 스크롤해서 다음 데이터를 찾아오는 것이 가능하지만 대화 꼬리 이미지를 제거하기 못해 신규 메시지 형태로 표시한다.
	...
	//	}		
		...
		$(".chat-mail dl").off().on("click", function(e){		
			...
			gBody4.openMail(tunid, "body", tdb, target_server);
		});		
...
//		html += "			<div class='wrap-message' id='"+msgid+"' mid='"+msgid+"'>";
		...
		if (typeof(info.ex) != "undefined"){
			...
			html += "			<div><img style='width:20px;height:20px' src='./img/icon/common/"+bimg_path+"'></div>";		
		}		
		html += "				<div class='img-thumb2'><img src='./emoticons/"+info.msg+"' alt=''></div>";
		html += "			</div>";
		if (opt == "me"){
			if (!gBody.is_my_chat){
				html += "		<button class='ico btn-chat-more on'>더보기</button>";
...
//		};
	...
	"favorite_draw" : function(obj){	
		return false;
		if (gBody3.cur_window != "chat" && gBody3.cur_window != "abc2"){
			return false;
		}
...
//		}	
		...
		if (roominfo != ""){
			...
			if (cpx == "n"){
				...
				$("#chatroomtitle").html(gBody2.select_name);
			}	
		}		
...
//				}				
				...
				var emoticon_img = "<img style=' height:35px' src='./emoticons/"+info.wmsg+"' alt=''>";				
				html += "					<dd id='chat_msg_"+xid+"'>"+emoticon_img+"</dd>";		
				
				if (typeof(info.wmsg.msg) == "undefined"){
					alarm_data.wmsg =  "<img style=' height:35px' src='./emoticons/"+info.wmsg+"' alt=''>";
				}else{
					alarm_data.wmsg =  "<img style=' height:35px' src='./emoticons/"+info.wmsg.msg+"' alt=''>";
				}
	...
	"enter_chatroom_for_chatroomlist_popup_empty" : function (cid){
		...
		if (cid.substring(0,1) == "s"){			
			...
			gBody.cur_cid = cid;			
			$("#message_txt").val("");			
			var lists = window.opener.gBody.cur_room_att_info_list;
			var tkey = "";
			for (var i = 0 ; i < lists.length; i++){
				var list = lists[i];
			...
			if (bol){
				...
				var tkey2 = opener.gBody.cur_chat_user;
				var name = opener.gBody.cur_chat_name;
			...
			gBody.cur_room_att_info_list = window.opener.gBody.cur_room_att_info_list;			
			 			...
			 			gap.showConfirm({
			 				...
			 				callback: function(){
								...
								//	var url = "/w0.nsf/(agtCopyDoc)?openAgent&ms="+mailserver+"&mf="+maildbpath+"&unid="+unid+"&empno=" + empno + "&lan=" + lang;
									var url = gBody.make_mail_url(ms, mf, unid, empno, lan);
									$.ajax({
										type : "GET",
										dataType : "json",
										contentType : "application/json; charset=utf-8",
...
//					myDropzonexx.removeAllFiles();
	...
	"myinfo_draw" : function(){	
		...
		html += "				<div class='photo-wrap' style='border-radius:50%; background-image:url(" + gap.person_photo_url(userinfo) + "),url(img/none.jpg);'></div>";
		html += "			</div>";		
		...
		html += "			<ul>";
		html += "				<li>";
		html += "					<span class='tit'>ID</span>";
		html += "					<span class='item'>"+userinfo.id+"</span>";
		html += "				</li>";
		html += "				<li>";
		...
		$("#my_profile_image").off().on("click", function(e){
			gOrg.showUserDetailLayer(gap.userinfo.rinfo.ky);
		});		
	...
	"receive_new_msg" : function(msg){		
						...
						if (isNotificationSupported){
							Notification.requestPermission().then(function(result){
								if (result === "granted"){									
									...
									gBody.notification_alram("[" + gap.lang.chat + "] " + gap.lang.newmsg, pmsg, "messenger", xid, msg);
								}else{
		...
		if (msg.ky != gap.search_cur_ky()){
			if (gap.focus){
				...
				if (gBody3.cur_window == "chat" && gBody.cur_cid != msg.cid){
					gap.change_title("","");
				}				
			}			
		}	
		...
		if ( (gBody3.cur_window == "chat" && gBody.cur_cid == msg.cid) || gma.cur_cid_popup == msg.cid){			
...
//				}			
			...
			if (msg.ky != gap.userinfo.rinfo.ky){
				...
				if (gBody3.cur_window == "chat" || gBody3.cur_window == "channel"){
					gBody.unread_chat_count_check_realtime();
				}else{
...
//			    	previewurl = chatserver_domain + firstex.sf + "/thumbnail/" + firstex.sn;	
				...
				if (item.kind === 'file') {					
					gBody.retrieveImageFromClipboardAsBlob(event, function(imageBlob){
				        ...
				        if(imageBlob){			            
				    		...
				    		html += "	<canvas style='max-width:900px; max-height:500px' id='mycanvas' />";				    			
	...
	"img_open" : function(){
		...
		$(".ico.btn-file-view").off().on("click", function(e){			
			...
			gBody3.file_convert("", url, "", gBody.cur_cid, "chat", caller, key, filename);	
		});
	},
...
//			var url = opp.replace(/.*\s?url\([\'\"]?/, '').replace(/[\'\"]?\).*/, '');
	...
	"chatroom_info_change_last_msg_receive" : function(info){
		...
		if (info.ty == 5 || info.ty == 6){
			...
			$("#chat_time_" + rid).text(tm);		
			$("#li_time_main_" + rid).text(tm);		
			var emoticon_img = "<img style=' height:35px' src='./emoticons/"+info.msg+"' alt=''>";
	...
	"chatroom_info_change_last_msg" : function(sq, id, obj){
		...
		//	var rid = curroom_id.replace(/\^/gi,"_").replace(".","-spl-");
			...
			if (msginfo.ty == "msg"){
				...
				$("#li_time_main_" + rid).text(tm);			
				var emoticon_img = "<img style=' height:35px' src='./emoticons/"+msginfo.msg.msg+"' alt=''>";
				$("#li_msg_main_" + rid).html(emoticon_img);				
				if ($("#chat_msg_" + rid).next().attr("class") == "nav-attach"){
					$("#chat_msg_" + rid).next().remove();
				}	
	...
	//	_wsocket.temp_list_status(lists, 1, "chatroom");		
		...
		html += '				<div class="photo-wrap" style="background-image:url(' + gap.person_photo_url(user_info) + '),url(img/none.jpg);"></div>'
		...
		//	html += "				<div class='owner-thumb'>"+user_info.user_img+"</div>";
			html += '				<div class="photo-wrap" style="background-image:url(' + gap.person_photo_url(member) + '),url(img/none.jpg);"></div>'
			html += "				<span data-status='status_"+user_info.ky+"' class='status ' style='top:6px; left:7px'></span>";
			html += "				<button data-phone='phone_"+user_info.ky+"' type='button' style='position:absolute'></button>";
			html += "			</div>";
			html += "			<div class='office_right' style='width: 64%;'>";
...
//			gap.open_subwin(url, "1200","850", "yes" , "", "yes");				
	...
	//			updateOnContentResize: true
		...
		html += "			<div class='office_prof user'>";
	//	html += "				<div class='owner-thumb'>"+user_info.user_img+"</div>";
		html += '				<div class="photo-wrap" style="background-image:url(' + gap.person_photo_url(xuser_info) + '),url(img/none.jpg);"></div>'
		...
		html += "			<div class='office_prof user'>";
	//	html += "				<div class='owner-thumb'>"+user_info.user_img+"</div>";
		html += '				<div class="photo-wrap" style="background-image:url(' + gap.chat_photo_url() + '),url(img/none.jpg);"></div>'
		html += "			</div>";
		html += "			<div class='office_right' style='width: 64%;'>";
		html += "				<div class='office_mem_name'>";
...
//		html += "					<span class='rank'>"+jobtitle+"</span>";
	...
	"chat_profile_init" : function(obj){		
		...
		//	html += "				<div class='owner-thumb'>"+user_info.user_img+"</div>";
			...
			if (userinfo.ky != gap.userinfo.rinfo.ky){
				...
				html += "			<div class='office_prof user'>";
			//	html += "				<div class='owner-thumb'>"+user_info.user_img+"</div>";
				html += '				<div class="photo-wrap" style="background-image:url(' + gap.person_photo_url(userinfo) + '),url(img/none.jpg);"></div>'
	...
	"chat_profile_image_draw" : function(obj){		
		...
		for (var i = 0 ; i < len; i++){
			...
			html += "				<img style='width:75px; ' data1='"+info.nm+"' data2='" + downloadpath + "' data3='"+info.sz+"' data4='"+sq+"' src='"+preview_img+"' onerror='this.src=\"img/thm_img_s.png\"' alt='' />";
...
//			var url = $(this).children().attr("data2") + "/" + encodeURIComponent(fname);
		...
		//	helper: 'clone',
			...
			start : function(event, ui){
				...
				html += "<img style='width:90px; height:80px' data1="+iobj.attr("data1")+" data2="+iobj.attr("data2")+"  data3="+iobj.attr("data3")+"  src=" +iobj.attr("src")+ " onerror='this.src=\"img/thm_img_s.png\"'>";
...
//		}
	...
	"open_add_member_search_layer" : function(opt){		
		...
		$("#search_window_close").on("click", function(event){			
			if (gBody3.cur_window == "drive"){
				//드라이브에서 다중대화 요청후 닫기를 클릭한 경우
				$("#sub_search_profile").hide();
				if (gBody2.select_folder_code != "root"){
					//폴더를 보고 있던 중이라 판단한다.
					$("#em_folder_"+ gBody2.select_folder_code).click();
				}else{
					$("#btn_fold_"+ gBody2.select_drive_code).click();
				}				
...
//		html2 += "		<button id='view_mode_gallery' class='ico btn-mode-gallery" + (gBody2.disp_view_mode == "gallery" ? " on" : "") + "'>갤러리보기</button>";
		...
		// 에디터 전환 버튼
		html2 += "	<div id='btn_editor_change' class='btn-editor-chg'><span>" + gap.lang.change_editor + "</span><img src='./img/icon/ico_editor_trans.png'/></div>";
	...
	//	html2 += "		<button class='btn-choose ico' id='other_btn'></button>";
		html2 += "		<button class='abs ico ico-plus' id='other_btn'></button>";
	...
	//	html2 += "		<button class='btn-emoticon ico' id='open_emoticon2'></button>";
		html2 += "		<button class='abs ico ico-emoticon' id='open_emoticon2'></button>";
		...
		if (!gBody3.select_files_tab){
			gBody.show_conversation_config();			
		}else{
		//	gBody.hide_conversation_config();
		}		
...
//	};
	...
	"channel_init" : function(selectid){		
		...
		//	formData.append("content", msg);
			...
			formData.append("channel_code", gBody.select_channel_code2);
			formData.append("channel_name", gBody.select_channel_name2);
		...
		$("#fileupload_start").off().on("click", function(e){		
			...
			gBody.select_channel_code2 = sel_val;
			gBody.select_channel_name2 = sel_txt;		
			if (gBody3.popup_status == "file"){				
				...
				if (gBody3.edit_mode == "T"){
					...
					if (drop_file_count > 0){
						...
						if (gBody3.delete_file_list.length > 0){
							isS = true;
							gBody.sub_file_delete_send_server();
						}else{
							myDropzone_channel.processQueue();
						}			
					}else{
					...
					/*	var content = $("#fileupload_content").val();
						//if (gBody3.delete_file_list.length > 0){							
						gBody.sub_file_delete_send_server2(content);*/						
						//mention 처리 /////////////////////////////////////////////////////////////
						var mentions_msg = "";
						var mentions_data = "";
						$("#fileupload_content").mentionsInput('val', function(text){
						...
						///////////////////////////////////////////////////////////////////////////						
						gBody.sub_file_delete_send_server2(mentions_msg, mentions_data);
					}
				}else{
					//업로드 할 파일이 있는지 체크한다.					
				...
				smsg.type = "ms";
			//	smsg.key1 = gBody.select_channel_code;
				smsg.key1 = sel_val;
				smsg.key2 = "";
				smsg.key3 = gBody.select_channel_name2;
				smsg.fr = gap.userinfo.rinfo.nm;
				//현재 채널방에 멤버리스트와 Owner값을 합치고 본인을 제거한 리스트를 가져온다.
			//	smsg.sender = gBody3.search_cur_channel_member(gBody.select_channel_code).join("-spl-");	
...
//					"channel_name" : gBody.select_channel_name,
		...
		$("#open_emoticon2_backup").off().on("click", function(){
			//채팅창에서 개인 설정 선택한 경우			
			$("#editor_close").click();			
			var html = "";
			html += "<div class='layer-emoticon' >";
			...
			html += "	<button class='ico btn-article-close'>닫기</button>";
			html += "	<ul class='list-emoticon' id='emoti_dis' style='overflow:hidden'>";			
			html += "		<li><img src='"+cdbpath+"/emoticons/rio.gif' data='rio.gif'></li>";
			html += "		<li><img src='"+cdbpath+"/emoticons/lala.gif' data='lala.gif'></li>";					
			for (var k = 1; k < 41; k++){
				html += "		<li><img src='"+cdbpath+"/emoticons/"+k+".gif' data='"+k+".gif'></li>";
			}
			html += "	</ul>";
			html += "</div>";							
			$("#open_emoticon2").qtip({
		...
		$("#channel_add_member").off().on("click", function(e){			
			var cinfo = gap.cur_room_search_info(gBody.select_channel_code2);
			if (cinfo.owner.ky == gap.userinfo.rinfo.ky){
				gBody2.modify_channel(gBody.select_channel_code2);
			}else{
		...
		$("#conv_config").off().on("click", function(e){		
			...
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='prevent_auto_scrolling' class='with-gap' type='radio' value='1'" + (gBody.prevent_auto_scrolling == "1" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.use+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='prevent_auto_scrolling' class='with-gap' type='radio' value='2'" + (gBody.prevent_auto_scrolling == "2" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.not_used+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "	</div>";			
			...
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='collapse_reply' class='with-gap' type='radio' value='1'" + (gBody.collapse_reply == "1" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.use+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='collapse_reply' class='with-gap' type='radio' value='2'" + (gBody.collapse_reply == "2" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.not_used+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "	</div>";			
			...
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='collapse_editor' class='with-gap' type='radio' value='1'" + (gBody.collapse_editor == "1" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.use+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='collapse_editor' class='with-gap' type='radio' value='2'" + (gBody.collapse_editor == "2" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.not_used+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "	</div>";			
			...
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='post_view_type' class='with-gap' type='radio' value='1'" + (gBody.post_view_type == "1" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.by_modified_date+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='post_view_type' class='with-gap' type='radio' value='2'" + (gBody.post_view_type == "2" ? " checked" : "") + ">";
			...
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='push_receive' class='with-gap' type='radio' value='1'" + (gBody.push_receive == "1" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.receive+"</span>";
			html += "			</label>";
			html += "		</div>";
			html += "		<div class='radio'>";
			html += "			<label>";
			html += "				<input name='push_receive' class='with-gap' type='radio' value='2'" + (gBody.push_receive == "2" ? " checked" : "") + ">";
			html += "				<span>"+gap.lang.do_not_receive+"</span>";
			html += "			</label>";
			html += "		</div>";	
			html += "	</div>";			
				...
				events : {
					show : function(event, api){
						...
						$("[name=prevent_auto_scrolling]").on("click", function(){
							gBody.prevent_auto_scrolling = $(this).val();
							gBody.save_channel_option(false);
						//	localStorage.setItem('prevent_auto_scrolling', $(this).val());
						});						
						$("[name=collapse_reply]").on("click", function(){							
							gBody.collapse_reply = $(this).val();
							gBody.save_channel_option(false);
							if (gBody.collapse_reply == "2"){
								//댓글 접기 사용 안함
						...
						$("[name=collapse_editor]").on("click", function(){							
							gBody.collapse_editor = $(this).val();
							gBody.save_channel_option(false);
							if (gBody.collapse_editor == "2"){
								//에디터 접기 사용 안함
						...
						$("[name=post_view_type]").on("click", function(){
							gBody.post_view_type = $(this).val();
							gBody.save_channel_option(false);
							$("#channel_list").empty();
							gBody3.draw_channel_list();
						});						
						$("[name=push_receive]").on("click", function(){
							gBody.push_receive = $(this).val();
							gBody.save_channel_option(true);
						});
					},
					hidden : function(event, api){
					//	gBody.save_channel_option();
		...
		$(".ico.btn-more.driveinfo").off().on("click", function(){
			$.contextMenu({
				...
				build : function($trigger, options){
					...
					return {
						items: gBody2.drive_info_menu_content(is_drive_share)
					}
				}
			});			
		});		
...
//				"channel_name" : gBody.select_channel_name,
	...
	"create_buddy_group" : function(groupname){
				...
				if (grouplist[k].nm == groupname){
					if (typeof(grouplist[k].usr) != "undefined"){
						for (var u = 0 ; u < grouplist[k].usr.length; u++){		
							...
							gBody2.channel_add_user("B", grouplist[k].usr[u]);
						}
						break;
					}
				}
		...
		if (gBody.is_buddylist_search){
			for (var p = 0 ; p < gBody.temp_search_result.length; p++){
				if (gBody.temp_search_result[p].ky != gap.userinfo.rinfo.ky){
					gBody2.channel_add_user("B", gBody.temp_search_result[p]);
				}
		...
		$("#input_buddylist_user").keydown(function(evt){
			if (evt.keyCode == 13){
				...
				gBody2.channel_user_search("B", $("#input_buddylist_user").val());
					...
					}, 
					{
						...
						setItems:function(items) { /* 반환되는 Items */						
							...
							for (var i = 0; i < items.length; i++){
								var _res = gap.convert_org_data(items[i]);
								if (_res.ky.toLowerCase() != gap.userinfo.rinfo.ky.toLowerCase()){
									gBody2.channel_add_user('B', _res);
								}
							}
							gBody2.alert_aleady_select_user();
						}
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	"go_drive_view" : function(){		
		gBody3.cur_window = "drive";		
		$("#left_main").css("width","312px");
		$("#container").hide();		
		gBody.drive_left_draw();		
		$("#tab1").hide();
		$("#tab2").hide();
		$("#tab3").show();		
		$("#left_roomlist").hide();
		...
		$("#left_channel").show();
		gBody.show_drive();		
		$("#main_body").css("right","0px").css("left","366px").css("background","#F7F7F7");
	},
	
	"drive_left_draw" : function(){		
		var html = "";	
		html += "<div class='nav-wide' id='nav_left_menu'>";		
		html += "<div  class='mu_work' style='overflow:hidden; height:100%; background-color:white'>";		
		html += "<h2 class='left-title' style='border:0px'></h2>";
		html += "<div class='nav-channel' id='left_channel' style='display:none; overflow:hidden '>";
		html += "	<div class='filter' id='channel_filter'>";
		html += "		<ul>";
		html += "			<li><button class='ico btn-filter-ppt' title='ppt'></button></li>";
		html += "			<li><button class='ico btn-filter-word' title='word'></button></li>";
		...
		html += "	<div class='lnb' id='channel_option'>";
		html += "		<ul>";
		html += "			<li class='lnb-all on' id='allcontent_drive'><em><span class='ico'  style='margin-top:8px'></span></em></li>";
		html += "			<li class='lnb-upload' id='mycontent_drive'><em><span class='ico' style='margin-top:8px'></span></em></li>";
		html += "			<li class='lnb-share' id='sharecontent_drive'><em><span class='ico' style='margin-top:8px'></span></em></li>";
		html += "			<li class='lnb-bookmark' id='favoritecontent_drive'><em><span class='ico' style='margin-top:8px'></span></em></li>";
		html += "		</ul>";
		html += "	</div>";
		html += "	<div class='folder-area' id='left_channel_list' style='border-top:none; margin-top:0px; overflow:hidden; height:calc(100% - 200px)'>";
		html += "	</div>";	
		html += "<div id='left_drive_btn'>";		
		html +=	"	<button class='btn-work-add' id='create_drive_room_left'><span class='ico ico-plus' style='left:80px'></span> "+gap.lang.create_drive+"</button>";		
		html += "</div>";	
		html += "</div>";
		html += "<div class='nav-list room' id='left_mail' style='display:none; padding-right:5px; overflow:hidden'>";
		html += "	<div class='nav-list-top'>";
	...
	//	$("#nav_left_menu").html(html);	
		$("#left_main").html(html);			
		gBody.drive_left_event_handler();	
		///전체 보기에서는 우측 프레임을 닫는다.
		gBody.channel_right_frame_close();	
		//가장 우측 메뉴 제거한다.
		gBody.chat_right_menu_close();	
		//Drive메뉴 클릭시 처음 호출되는 함수 여기에 내가 올린 최신 리스트 순으로 목록을 표시해야 한다.
		gBody2.draw_first_list();
	},
	...
	"drive_left_event_handler" : function(){		
		...
		$("#channel_option li").off().on("click", function(e){			
			...
			if (id == "allcontent_drive"){
				gBody2.select_left_menu = "1";				
			}else if (id == "mycontent_drive"){
				gBody2.select_left_menu = "2";				
			}else if (id == "sharecontent_drive"){
				gBody2.select_left_menu = "3";				
			}else if (id == "favoritecontent_drive"){
				gBody2.select_left_menu = "4";
			}			
			gBody.channel_right_frame_close();			
			$("#ch_query").val("");
			gBody2.query_str = "";			
		...
		$("#channel_filter ul li button").on("click", function(e){
			...
			var filter = "";
			var pre_filter = gBody.click_filter_image;			
					...
					setTimeout(function(){							
						...
						setTimeout(function(){							
							if (typeof(gap.param2) != "undefined"){
								...
								//$("#" + gap.param2).click();
								gBody2.show_channel_data(gap.param2)
							};							
						}, 100);
					}, 500);					
				}else if (pa == "todo"){
	...
	"init_mention_userdata" : function(){
		$('#message_txt_channel').mentionsInput({
			onDataRequest:function (mode, query, callback) {
				var list = gBody3.search_channel_members(gBody3.cur_opt);
...
//				onTotalScrollOffset: 100,
	...
	"channel_main_draw" : function(){
		...
		//gBody.main_sort = 2; //업데이트 날자로 변경한다.
		...
		var end = "";
		if (gBody.main_start != ""){
			start = gBody.main_start;
			end = gBody.main_end;
		}		
		var sort = gBody.main_sort; //1이면 생성일 순으로 2이면 최신 업데이트 순으로		
		var postData = {
				"start" : start,
				"end" : end,
				"sort" : sort,
				"query" : gBody.main_query
			};
		$.ajax({
			type : "POST",
			url : surl,
			...
			success : function(__res){
				...
				if (res.result == "OK"){					
					...
					$(".left-area").css("width", "100%");
					$("#channel_main").show();					
					var html = gBody.draw_channel_main(list);
...
//							    endInput: '#todo_enddate',
							    ...
							    onChange : function(event, inst){
							    	if (event.value[1] != null){							    									   										
										...
										gBody.main_start = startdate;
										gBody.main_end = enddate;
										gBody.main_sort = "1";
										gBody.main_query = "";
							    		gBody.channel_main_draw();
							    		return false;
	...
	"draw_channel_main" : function(list){
		...
		html += "			<div class='xbtn-search-close' style='display:"+(gBody.main_query == '' ?  'none' : '')+"' id='main_query_btn_close'></div>";
		html += "		</div>";
		...
		html += "					<option value='1' + " + (gBody.main_sort == '1' ? 'selected' : '') + ">"+gap.lang.by_registration_date+"</option>";
		html += "					<option value='2' + " + (gBody.main_sort == '2' ? 'selected' : '') + ">"+gap.lang.u_date+"</option>";
		html += "					<option value='3' + " + (typeof(gBody.main_sort) == 'undefined' ? 'selected' : '') + ">"+gap.lang.todo_period+"</option>";
		html += "				</select>";
		html += "			</div>";
		html += "		</div>";
		html += "	</div>";
		...
		for (var i = 0 ; i < list.length; i++){
			var item = list[i];
			html += gBody.draw_channel_item(item);
		}
		html += "				</ul>";
		html += "			</div>";
		html += "		</div>";
	...
	"draw_channel_item" : function(obj){
		...
		html += "				<div class='photo-wrap' style='border-radius:50%; background-image:url(" + gap.person_photo_url(owner) + "),url(img/none.jpg);'></div>";
		html += "			</div>";
		html += "			<div>";
		...
		if (typeof(obj.member) != "undefined"){
			...
			for (var k = 0 ; k < len; k++){
				...
				html += "			<div class='photo-wrap' style='border-radius:50%; background-image:url(" + gap.person_photo_url(mm) + "),url(img/none.jpg);'></div>";
				html += "		</div>";
			}
			if (mcnt > 5){
				var bnt = mcnt - 5;
	...
	"lnb_menu_click" : function(id){		
		...
		if (id == "help"){
			gap.hide_new_layer();
			gBody3.cur_opt = ""; //현재 열려 있는 체널 값을 초기화 한다.
			gTop.show_manual();
			return;
