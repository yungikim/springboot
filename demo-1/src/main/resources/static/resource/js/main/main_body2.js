...
function gBodyFN2(){
	...
	this.menu_length = 19;
	try{
		this.is_dev = document.location.host.search(/devportal/i) != -1;
	}catch(err){}
} 
...
gBodyFN2.prototype = {
	...
	"draw_first_list" : function(){
		...
		gBody2.select_left_menu = "1";	//1-전체파일, 2-내가올린 파일, 3-공유된 파일
		gBody2.draw_main_data(1);
	},
	...
	"draw_main_data" : function(page_no){
		...
		if (page_no == 1){
			...
			gBody2.start_page = "1";
			gBody2.cur_page = "1";
			gBody2.per_page = (gBody2.disp_view_mode == "list" ? "10" : "20");
			gBody2.cur_file_count = 0;
			gBody2.cur_file_total_count = 0;			
			var title = "";
			if (gBody2.select_left_menu == "1"){
				title = gap.lang.allcontent_drive;
				
			}else if (gBody2.select_left_menu == "2"){
				title = gap.lang.mycontent_drive;
				
			}else if (gBody2.select_left_menu == "3"){
			...
			html += '				<input type="text" autocomplete="off" value="' + gBody2.query_str + '" id="ch_query" placeholder="' + gap.lang.input_search_query + '" />';
			html += '			</div>';
			html += '			<button class="ico btn-channel-search" id="ch_query_btn">검색</button> ';
			html += '		</div>';
			html += '		<div class="drive-btns">';
			if (gBody2.select_left_menu != "3"){
			...
			html += '			<button id="view_mode_list" class="ico btn-mode-list' + (gBody2.disp_view_mode == "list" ? " on" : "") + '"><span>리스트보기</span></button>';
			html += '			<button id="view_mode_image" class="ico btn-mode-img' + (gBody2.disp_view_mode == "image" ? " on" : "") + '"><span>이미지보기</span></button>';
	//		html += '			<button id="view_mode_gallery" class="ico btn-mode-gallery' + (gBody2.disp_view_mode == "gallery" ? " on" : "") + '"><span>갤러리보기</span></button>';
			html += '		</div>';
			html += '	</div>';			
		...
		var postData = {
				"start" : (gBody2.start_skp - 1).toString(),
				"perpage" : gBody2.per_page,
				"q_str" : gBody2.query_str,
				"dtype" : gBody3.click_filter_image,
				"type" : gBody2.select_left_menu
			};
		$.ajax({
			type : "POST",
			url : surl,
			...
			success : function(__res){
				...
				if (res.result == "OK"){
					gBody2.cur_file_count += res.data.filelist.length;
					gBody2.cur_file_total_count = res.data.totalcount;
					gBody2.draw_main_data_list(page_no, res.data);						
				}else{
					if (gBody2.query_str != "" && res.filelist == null){
						gBody2.cur_file_count = 0;
						gBody2.cur_file_total_count = 0;
						gBody2.draw_main_data_list(1, {data:[], totalcount:0});	
						return false;						
					}else{
		...
		$("#ch_query").keypress(function(e){
			if (e.keyCode == 13){
				gBody2.search_enter(gBody2.select_left_menu);
			}
		});
	...
	"draw_main_data_list" : function(page_no, res){
		...
		if (gBody2.query_str != "" && res.filelist.length == 0){
			//검색된 파일이 없는 경우
			var html = '<div class="msg-empty"><img src="' + cdbpath + '/img/empty.png" alt="" />' + gap.lang.searchnoresult + '</div>';
			$("#wrap_main_data_list").empty();
			$("#wrap_main_data_list").html(html);
			return false;
		}
		...
		for (var k = 0;  k < res.filelist.length; k++){
			...
			if (gBody2.disp_view_mode == "list"){
				file_html += '	<li id="' + file_id + '" style="list-style:none;' + (gBody2.select_left_menu == "3" ? "padding-left:60px;'" : "") + '">';
				if (gBody2.select_left_menu != "3"){
				...
				file_html += '			<span class="ico ico-file ' + icon_kind + '"' + (gBody2.select_left_menu == "3" ? " style='left:20px'" : "") + '></span>';
				file_html += '			<dl>';
			...
			//	file_html += '	<div class="thm' + (icon_kind == "video" ? " video-thm" : "") + '" id="file_' + file_id + '" fserver="' + file_info.fserver + '" fname="' + file_info.filename + '" md5="' + file_info.md5 + '" furl="' + downloadurl + '" thmok="' + file_info.thumbOK + '">';
				...
				if (gBody2.disp_view_mode == "image" && icon_kind == "img"){
					show_thumb = true;
				}
				if (gBody2.disp_view_mode == "gallery" && file_info.thumbOK == "T"){
					show_thumb = true;
				}				
				if (show_thumb){
					file_html += '		<img src="' + gap.search_file_convert_server(file_info.fserver) + '/FDownload_thumb.do?id=' + file_id + '&ty=' + (gBody2.select_left_menu == "4" ? "3" : "1") + '">' + (icon_kind == "video" ? '<span class="ico ico-video-b">재생</span>' : "");	
				}else{
				...
				$.ajax({
					...
					success : function(res){
						if (res.result == "OK"){
							...
							gBody2.draw_file_detail_info(file_info, "1");	
		...
		$(".ico.btn-more.main-file-menu").on("click", function(){
			$.contextMenu({
				...
				callback : function(key, options){
					var $selector = (gBody2.disp_view_mode == "list" ? $(this).prev() : $(this).prev().prev());
					...
					gBody2.context_menu_call_file_mng(key, options, f_id, f_server, f_name, f_md5, f_url);
				},
				events : {
				...
				build : function($trigger, options){
					var $selector = (gBody2.disp_view_mode == "list" ? $trigger.prev() : $trigger.prev().prev());
					...
					return {
						items: gBody2.file_menu_content(is_preview, fowner)
					}
				}
			});
		});			
		...
		//페이징
		gBody2.total_file_count = res.totalcount;
		gBody2.total_page_count = gBody2.get_page_count(gBody2.total_file_count, gBody2.per_page);
		if (gBody2.disp_view_mode == "list"){
			gBody2.initialize_page();
		}
		...
		//	setTop : ($("#wrap_favorite_data_list").height()) + "px",
			callbacks : {
				...
				onTotalScroll: function(){
					if (gBody2.disp_view_mode != "list"){
						page_no++;
						gBody2.add_main_data_list(page_no);
					}
				},
	...
	"add_main_data_list" : function(page_no){
		...
		if (gBody2.disp_view_mode == "list"){
			...
			if (gBody2.cur_file_total_count > gBody2.cur_file_count){
				is_continue = true;
			}
		}
		if (is_continue){
	...
	"draw_main_sub_folder" : function(res){
		for (var i = 0; i < res.length; i++){
			...
			gBody2.__tree_folder_event(drive_key, folder_id);
		}
	},
	...
	"get_folder_fullpath" : function(_selector, _path, _title){
		if (_selector.attr("data") == "root"){
			...
			if (_selector.parent().parent().siblings("div").attr("data") == "root"){
				...
				return gBody2.get_folder_fullpath(_ele, _fullpath, _fulltitle);
			}		
		}
	},
	...
	"get_file_path" : function(drive_code, folder_code){
		if (folder_code == "root"){
			for (var i = 0; i < gBody.cur_drive_list_info.length; i++){
				var drive_info = gBody.cur_drive_list_info[i];
		...
		$("#main_" + folder_id).on("click", function(e){
			...
			if (e.target.className == "ico btn-more folder-menu"){
				$.contextMenu({
					...
					trigger : "left",
					callback : function(key, options){
						gBody2.context_menu_call_folder_mng(key, options, $(this).parent().attr("id"));
					},
					events : {
						hide: function (options) {
							$(this).removeClass("on");
					...
					build : function($trigger, options){
						options.handleObj.data.zIndex = parseInt(gap.maxZindex()) + 1;
						return {
							items: gBody2.folder_menu_content()
						}
					}
				});				
				...
				var full_path_info = gBody2.get_folder_fullpath($(this), $(this).attr("id").replace("main_", ""), $("#" + $(this).attr("id") + " em").text());
				...
				gBody2.title_path_code = [];
				gBody2.title_path_name = [];				
				for (var j = 1; j < full_code.length; j++){
					gBody2.title_path_code.push(full_code[j]);
				}
				for (var k = 1; k < full_name.length; k++){
					gBody2.title_path_name.push(full_name[k]);
				}				
				id = drive_key;
				var parent_folder_id = $(this).parent().parent().siblings("div").attr("id").replace("main_", "");
				var cur_folder_id = $(this).attr("id").replace("main_", "");				
				gBody2.select_drive_code = $("#" + $(this).attr("id")).attr("data");
				gBody2.select_drive_name = $("#main_" + gBody2.select_drive_code + " em").text();
				...
				gBody2.select_folder_code = folder_id;
				gBody2.select_folder_name = $("#" + $(this).attr("id") + " em").text();
				gBody3.draw_folder_members();
			//	gBody3.draw_drive_members(gBody2.select_drive_code);	// 폴더 클릭 시 드라이버 멤버 대신 폴더 멤버 표시함 - 2021.11.16
	...
	"create_chatroom" : function(){
		var html = gBody2.new_chat_layer_html();		
		gap.showBlock();
		...
		$("#input_chat_user").keydown(function(evt){
			if (evt.keyCode == 13){
				...
				gBody2.channel_user_search("T", $("#input_chat_user").val(), true);				
			}
		})
			...
			window.ORG.show(
					...
					}, 
					{
						...
						setItems:function(items) { /* 반환되는 Items */
							...
							gBody2.aleady_select_user_count = 0;
							...
							gBody2.alert_aleady_select_user();
						}
					});
		});
		...
		$("#create_chat_btn").on("click", function(){	
			...
			gBody2.select_name = select_name;
	...
	"create_drive" : function(obj, flag){
		...
		var html = gBody2.drive_layer_html(is_update, is_drive_share);		
		gap.showBlock();
	//	$(html).appendTo('body');
		$("#common_work_layer").show();
		$("#common_work_layer").html(html);		
		...
		if (is_update){
			//드라이브 정보 update인 경우
			$("#input_drive").val(gap.textToHtml(obj.ch_name));
			$("#input_drive").parent().find("label").addClass("on");
			if (obj.ch_share == "Y"){
				gBody2.aleady_select_user_count = 0;
				for (var i = 0; i < obj.member.length; i++){
					gBody2.channel_add_user("D", obj.member[i]);
				}
				gBody2.alert_aleady_select_user();
			}
		}		
		$("#input_drive_user").keydown(function(evt){
			if (evt.keyCode == 13){
				...
				gBody2.channel_user_search("D", $("#input_drive_user").val());
			}
		})
		.bind('paste', function(e){
			gap.change_paste_text(e, this);
					...
					}, 
					{
						...
						setItems:function(items) { /* 반환되는 Items */
							...
							for (var i = 0; i < items.length; i++){
								var _res = gap.convert_org_data(items[i]);
								if (_res.ky.toLowerCase() != gap.userinfo.rinfo.ky.toLowerCase()){
									gBody2.channel_add_user('D', _res);
								}
							}
							gBody2.alert_aleady_select_user();
						}
					...
					if (res.result == "OK"){
						...
						if (!gBody3.check_top_menu_new()){
							if (gBody2.select_folder_code == "root"){
								if (gBody2.select_drive_code == ch_code){
									$("#drive_title_path span").html(channel_name);
								}
								
							}else{
								if (gBody2.select_drive_code == ch_code){
									$("#title_" + gBody2.select_drive_code).html(channel_name);
								}
							}
						}						
						if (gBody2.select_drive_code == ch_code){
							gBody2.update_drive_info(ch_code);
	...
	"show_drive_data" : function(drive_code){
		id = drive_code;
		gRM.box_layer_close();
		gBody2.select_left_menu = "";
		gBody2.select_drive_code = drive_code;
		gBody2.select_drive_name = $("#main_" + drive_code + " em").text();
		gBody2.select_folder_code = "root";
		gBody2.select_folder_name = "";
		gBody2.title_path_name = [];
		gBody2.title_path_code = [];
		gBody2.query_str = "";
		gBody2.per_page = (gBody2.disp_view_mode == "list" ? "10" : "20");
		gBody3.cur_opt = "";
		gBody3.draw_drive_members(drive_code);
		gBody2.draw_drive_data(1);
		gBody3.removeClass_channel();
	...
	"context_menu_call_drive_mng" : function(key, options, _id){
		...
		if (key == "modify"){
			gBody2.modify_drive(_id);
			
		}else if (key == "delete"){
			gBody2.delete_drive(_id);
			
		}else if (key == "exit"){
			gBody2.exit_drive(_id);
			
		}else if (key == "create_share"){
			gBody2.create_drive(null, 'Y');
			
		}else if (key == "create_person"){
			gBody2.create_drive(null, 'N');	
			
		}else if (key == "exit_list"){
			gBody2.draw_exit_info_list("1");				
		}
	},
	...
	"modify_drive" : function(ch_code){
		...
		$.ajax({
			...
			success : function(res){
				if (res.result == "OK"){
					gBody2.create_drive(res.data, res.data.ch_share);
	...
	"delete_drive" : function(ch_code){
		...
		gap.showConfirm({
			...
			$.ajax({
				...
				success : function(res){
					if (res.result == "OK"){
						var member_data = $.map(gBody.cur_drive_list_info, function(ret, key) {
	...
	"update_drive_info" : function(ch_code){
		...
		$.ajax({
			...
			success : function(res){
				...
				gBody.cur_drive_folder_list_info = res.folder;
				
				if (is_member_update){
					// 변경된 멤버 바로 표시
					if (gBody2.select_drive_code != ""){
						// 드라이브가 선택된 경우만 멤버 표시
						gBody3.draw_drive_members(ch_code);
					}
				}
			},
			error : function(e){
	...
	"update_drive_channel_html" : function(res){		
		...
		if (data.type == "deld"){
			...
			$("#share_drive_list .drive-code").on("click", function(e){
				if (e.target.className == "ico btn-more drive-area"){
					$.contextMenu({
						...
						trigger : "left",
						callback : function(key, options){
							gBody2.context_menu_call_drive_mng(key, options, $(this).parent().attr("id"));
						},
						events : {
							hide: function (options) {
								$(this).removeClass("on");
						...
						build : function($trigger, options){
							return {
								items: gBody2.drive_menu_content($($trigger).parent().attr("owner"))
							}
						}
					});
				}else{
					var id = e.currentTarget.id;
					gBody2.show_drive_data(id);
				}
			});			
			gBody2.update_drive_info();				
			...
			if (data.ch_name.length > gBody2.menu_length){
				dischname = data.ch_name.substring(0,gBody2.menu_length) + "...";
			}
	...
	"__channel_layer_html" : function(is_update, is_channel_share){
		// 레이어 높이 설정		
		var html = "";
		if (is_channel_share == "Y"){
			html += '<h2>' + (is_update ? gap.lang.update_share_channel_info : gap.lang.create_share_channel) + '</h2>';
			
		}else{
			html += '<h2>' + (is_update ? gap.lang.update_person_channel_info : gap.lang.create_person_channel) + '</h2>';
		}
		html += '<button class="ico btn-close" id="close_channel_layer">닫기</button>';
		html += '<div class="input-field">';
		html += '	<input type="text" class="formInput" id="input_channel" autocomplete="off">';
		html += '	<label for="input_channel" class="">' + gap.lang.channel_name + '</label>';
		html += '	<span class="bar"></span>';
		html += '</div>';
		html += '<div class="input-field">';
		html += '	<input type="text" class="formInput" id="input_channel_user" autocomplete="off">';
		html += '	<label for="input_channel_user" class="">' + gap.lang.invite_user + '</label>';
		html += '	<span class="bar"></span>';
		if (isShowOrg == "T"){
			html += '	<button id="org_pop_btn" class="ico btn-org"></button>';
		}
		html += '</div>';
		html += '<div class="channel-invite-user" id="channel_member_list">';
		html += '</div>';
		html += '<ul class="same-name" style="top:160px; display:none;" id="same_name_list">';
		html += '</ul>';
		html += '<div class="layer-bottom">';
		html += '	<button class="btn-point" id="create_channel_btn"><strong>' + (is_update ? gap.lang.basic_save : gap.lang.basic_make) + '</strong></button>';
		html += '</div>';		
		return html;
	},	
	
	"channel_layer_html" : function(is_update, is_channel_share){
		var html = "";		
		html += '<div id="update_channel_layer" class="layer_wrap center work-opt">';
		html += '	<div class="layer_inner">';
		html += '		<div id="close_channel_layer" class="pop_btn_close"></div>';
		if (is_channel_share == "Y"){
			html += '		<h4>' + (is_update ? gap.lang.update_share_channel_info : gap.lang.create_share_channel) + '</h4>';
		}else{
			html += '		<h4>' + (is_update ? gap.lang.update_person_channel_info : gap.lang.create_person_channel) + '</h4>';
		}		
		html += '		<div class="layer_cont_wr">';		
		html += '			<div class="layer_cont left">';
		html += '				<div class="cont_wr rel">';
		html += '					<h5>' + gap.lang.channel_name + '</h5>';
		html += '					<div class="before_select">';
		html += '						<input type="text" id="input_channel" autocomplete="off" class="input" placeholder="' + gap.lang.input_channel_name + '">';
		html += '					</div>';
		html += '				</div>';
		html += '				<div class="cont_wr rel" style="height: 230px">';
		html += '					<h5>' + gap.lang.invite_member + '</h5>';
		html += '					<div class="before_select f_between">';
		html += '						<input type="text" id="input_channel_user" class="input" autocomplete="off"  placeholder="' + gap.lang.input_invite_member + '">';
		html += '						<button id="org_pop_btn" class="type_icon"></button>';
		html += '					</div>';
		html += '					<!-- 선택하면 나오면 화면 -->';
		html += '					<div class="after_select until_wr" style="display:none;">';
		html += '						<ul id="channel_member_list" class="scroll until p5" style="max-height:125px; overflow-y:auto;">';
		html += '						</ul>';
		html += '					</div>';
		html += '				</div>';
		html += '			</div>';		
		html += '			<div class="layer_cont left grant">';
		html += '				<div class="cont_wr rel" style="width:320px;">';
		html += '					<h5>' + gap.lang.cont_reg + '</h5>';
		html += '					<div class="before_select">';
		html += '						<input type="radio" id="work_opt_reg_on" name="work_opt_reg" value="all" checked><label for="work_opt_reg_on" style="margin-right:15px;">' + gap.lang.reg_all + '</label>';
		html += '						<input type="radio" id="work_opt_reg_off" name="work_opt_reg" value="user"><label for="work_opt_reg_off">' + gap.lang.reg_user + '</label>';
		html += '					</div>';
		html += '					<div id="reg_opt_wr" style="display:none;">';
		html += '						<div class="before_select f_between" style="margin-top:8px;">';
		html += '							<input type="text" id="input_reg_user" class="input" autocomplete="off"  placeholder="' + gap.lang.reg_user_ph + '">';
		html += '							<button id="btn_work_org" class="type_icon"></button>';
		html += '						</div>';
		html += '						<div class="after_select until_wr" style="display:none;">';
		html += '							<ul id="reg_member_list" class="scroll until p5" style="max-height:125px; overflow-y:auto;">';
		html += '							</ul>';
		html += '						</div>';
		html += '					</div>';
		html += '				</div>';
		html += '				<div class="cont_wr rel">';
		html += '					<h5>' + gap.lang.cont_del + '</h5>';
		html += '					<div class="before_select">';
		html += '						<input type="radio" id="work_opt_del_on" name="work_opt_del" value="T"><label for="work_opt_del_on" style="margin-right:15px;">' + gap.lang.enable + '</label>';
		html += '						<input type="radio" id="work_opt_del_off" name="work_opt_del" value="F" checked><label for="work_opt_del_off">' + gap.lang.disable + '</label>';
		html += '					</div>';
		html += '				</div>';
		html += '				<div class="cont_wr rel">';
		html += '					<h5>' + gap.lang.cont_copy + '</h5>';
		html += '					<div class="before_select">';
		html += '						<input type="radio" id="work_opt_copy_on" name="work_opt_copy" value="T" checked><label for="work_opt_copy_on" style="margin-right:15px;">' + gap.lang.enable + '</label>';
		html += '						<input type="radio" id="work_opt_copy_off" name="work_opt_copy" value="F"><label for="work_opt_copy_off">' + gap.lang.disable + '</label>';
		html += '					</div>';
		html += '				</div>';
		html += '				<div class="cont_wr rel">';
		html += '					<h5>' + gap.lang.opt_reply + '</h5>';
		html += '					<div class="before_select">';
		html += '						<input type="radio" id="work_opt_reply_on" name="work_opt_reply" value="T" checked><label for="work_opt_reply_on" style="margin-right:15px;">' + gap.lang.enable + '</label>';
		html += '						<input type="radio" id="work_opt_reply_off" name="work_opt_reply" value="F"><label for="work_opt_reply_off">' + gap.lang.disable + '</label>';
		html += '					</div>';
		html += '				</div>';
		html += '			</div>';		
		html += '		</div>';	// END layer_cont_wr 		
		html += '		<div class="btn_wr" style="margin-top:30px;">';
		html += '			<button id="create_channel_btn" class="btn_layer confirm">' + (is_update ? gap.lang.basic_save : gap.lang.create) + '</button>';
		html += '		</div>';
		html += '	</div>';
		html += '</div>';		
		return html;
	...
	"create_channel" : function(obj, id, flag, callback){
		var is_update = (obj != undefined ? true : false);
		var folderkey = (id != undefined ? id : "");
		var is_channel_share = (flag != undefined ? flag : "");
		if (is_update){
			folderkey = (obj.folderkey != undefined ? obj.folderkey : "");
		}		
		var html = gBody2.channel_layer_html(is_update, is_channel_share);		
		...
		if ($('#blockui').is(':visible')) {
			...
			$("header, div, article, section, nav, aside").not("[class*='mbsc-']").each(function(){
				...
				if (ck_z == z) {
					gBody2.is_before_block = true;
					gBody2.top_layer_element = $(this);
					return false;
				}
			});
		}		
		...
		$("#common_work_layer").html(html);		
		$("#input_channel").focus();		
		var $layer = $('#update_channel_layer');
		var inx = parseInt(gap.maxZindex()) + 1;
		$layer.css('z-index', inx).addClass('show-layer');
		if (is_update){
			//채널 정보 update인 경우
			$("#input_channel").val(gap.textToHtml(obj.ch_name));
			$("#input_channel").parent().find("label").addClass("on");
			if (obj.ch_share == "Y"){
				gBody2.aleady_select_user_count = 0;
				for (var i = 0; i < obj.member.length; i++){
					gBody2.channel_add_user("C", obj.member[i]);
				}
				gBody2.alert_aleady_select_user();
			}			
			// 옵션 처리
			if (obj.opt_reg){
				...
				if (obj.opt_reg == 'all'){
					...
					$.each(obj.opt_reg_list, function(){
						gBody2.channel_add_user("CO", this);
					});
				}
			}
		}
		$("#input_channel_user").keydown(function(evt){
			if (evt.keyCode == 13){
				if ($("#input_channel_user").val().trim() == ""){
					mobiscroll.toast({message:gap.lang.input_invite_user, color:'danger'});
					return false;
				}				
				gBody2.channel_user_search("C", $("#input_channel_user").val());				
			}
		})
		.bind('paste', function(e){
			gap.change_paste_text(e, this);
					...
					}, 
					{
						...
						setItems:function(items) { /* 반환되는 Items */
							...
							for (var i = 0; i < items.length; i++){
								var _res = gap.convert_org_data(items[i]);
								if (_res.ky.toLowerCase() != gap.userinfo.rinfo.ky.toLowerCase()){
									gBody2.channel_add_user('C', _res);
								}
							}
							gBody2.alert_aleady_select_user();
						}
		...
		$("#input_reg_user").keydown(function(evt){
			if (evt.keyCode == 13){
				if ($(this).val().trim() == ""){
					mobiscroll.toast({message:gap.lang.input_invite_user, color:'danger'});
					return false;
				...
				gBody2.channel_user_search("CO", $(this).val());				
			}
		})
		.bind('paste', function(e){
			gap.change_paste_text(e, this);
		});
		$("#btn_work_org").on("click", function(){
			window.ORG.show(
					{
					...
					}, 
					{
						...
						setItems:function(items) { /* 반환되는 Items */
							for (var i = 0; i < items.length; i++){
								var _res = gap.convert_org_data(items[i]);
								if (_res.ky.toLowerCase() != gap.userinfo.rinfo.ky.toLowerCase()){
									gBody2.channel_add_user('CO', _res);
								}
							}
						}
					});
		...
		$("#create_channel_btn").on("click", function(){
			if ($("#input_channel").val().trim() == ""){
				...
				success : function(ress){
					...
					if (res.result == "OK"){						
						...
						if (channel_name.length > gBody2.menu_length){
							dischname = channel_name.substring(0,gBody2.menu_length) + "...";
						}						
						if (is_update){	
							_html += '<span class="ico ico-' + (member_count != 0 ? "people" : "person") + '"></span><em>' + dischname + '</em><button class="ico btn-more channel-area">더보기</button>';
							$("." + obj.ch_code).empty();
							$("." + obj.ch_code).append(_html);							
							if (new_member_ky.length > 0){
								var _obj = new Object();
								_obj.sender = new_member_ky;
								_obj.ch_code = obj.ch_code;
								_obj.ch_name = gap.textToHtml(channel_name);
								_obj.channel_name = gap.textToHtml(channel_name);
								_obj.channel_code = obj.ch_code;
								_obj.folderkey = folderkey;
								_obj.owner = gap.userinfo.rinfo.ky;
								_obj.type = "cc";
								_wsocket.send_box_msg(_obj, 'chmng');
							}
							...
							_html += '		<li id="li_cl_' + res.ch_code + '" style="padding-left:45px;">';
							_html += '			<div id="' + res.ch_code + '" owner="' + gap.search_cur_ky() + '" class="channel-code"><span class="ico ico-' + (member_count != 0 ? "people" : "person") + '"></span><em>' + dischname + '</em><button class="ico btn-more channel-area">더보기</button></div>';
							_html += '		</li>';					
							if (folderkey != ""){
								if ($("#li_" + folderkey).length == 0){
								...
								//	}
									...
									if ($("#li_" + folderkey).find("li").length == 0){
										var _ul_html = '<ul id="ul_' + folderkey + '">' + _html + '</ul>';
										$("#li_" + folderkey).append(_ul_html);
							
									}else{
										$("#ul_" + folderkey).append(_html);
										$("#ul_" + folderkey).show();	// 생성된 채널 표시
									}
									
									$("#btn_fold_" + folderkey).html('<button class="btn-fold on"></button>');
									$("#" + res.ch_code).css("margin-left", "10px");
								}
						...
						gBody2.hide_create_channel();						
						var ch_code = (is_update ? obj.ch_code : res.ch_code);						
						if (gBody3.select_channel_code == ch_code){
							gBody2.update_channel_info(is_update ? obj.ch_code : res.ch_code);							
						}else{
							gBody2.update_channel_info();
						}						
						// callback설정한 경우 callback함수 수행 후 리턴처리 (메인에서 간편 업무 등록으로 채널 생성하는 경우)
						...
						if (!gBody3.check_top_menu_new()){
							if (gBody3.select_channel_code == ch_code){
							//	$("#channel_top_list .channel-header h2").html(channel_name + gBody3.expand_collapse_btn());	
								$("#channel_top_list .channel-header h2").html(channel_name );	
							}
						}					
						gBody3.main_re_draw();			
					}else{
		...
		$("#close_channel_layer").on("click", function(){
			//gap.remove_layer('update_channel_layer');
			gBody2.hide_create_channel();
		//	gap.remove_layer('common_work_layer');
		});		
	},
	
	"hide_create_channel" : function(){
		...
		$('#update_channel_layer').remove();
	},
	
	"show_channel_data" : function(channel_code, ob){
		//권한을 체크한다.
		
		var cinfo = gap.search_channel_info(channel_code);
		...
		gBody3.check_reddot(channel_code);		
		//gRM.box_layer_close();
		//탬클릭시 오류 나는 부분 처리
		gBody3.tab_click_event();
		//필터링 설정한 부분을 초기화 한다.
		gBody2.click_filter_image = "";
		$("#channel_filter ul li button").removeClass("on");	
		//채널명이 긴 경우 목록에 짤라서 표시하기 때문에 원 채널명을 가져오기 위해서 한번 검색한다.
		var info = gap.cur_room_search_info(channel_code);
		gBody3.select_channel_code = channel_code;
		gBody3.select_channel_name = info.ch_name;
		gBody3.select_channel_code2 = channel_code;
		gBody3.select_channel_name2 = info.ch_name;	
		gBody3.select_files_tab = false;
		gBody3.cur_opt = channel_code;
		...
		if (channel_code == gBody3.temp_room_key){
			//특정 업무방의 경우 업무지시 창이 먼저 표시되게 예외처리한다. caller 황수진 2023.07.03
			gBody3.draw_channel_list_request();	
		}else{
			gBody3.draw_channel_list();	
		}			
		gBody3.removeClass_channel();
		//우측 프레임에 채널 참석자 정보 표시한다.
		if (!gBody3.check_top_menu_box(channel_code)){
			//$("#channel_right").show();
			gBody3.draw_channel_members(channel_code);
		}
		$(".chat-bottom").show();		
		if (ob == "T"){
			$("#" + channel_code).parent().addClass("on");
		}else{
			if (typeof(gBody2.click_oob) == "undefined"){
			//	$("#" + channel_code).parent().addClass("on");
			}else{
				$(gBody2.click_oob.currentTarget).parent().addClass("on");
			}
		}		
		$("#ch_query").val("");
		$("#channel_top_list .channel-header h2").html(gBody3.select_channel_name2);
	},
	
	"channel_menu_content" : function(owner, obj){	
		var tid = $(obj).parent().attr("id");		
		gBody2.channel_menu_content_click_id = tid;		
		...
		//	items["sep02"] = "-------------";
		//	items["member"] = {name : gap.lang.member_info};
			items["sep03"] = "-------------";
			if (is_star){
	...
	"context_menu_call_channel_mng" : function(key, options, _id){
		...
		if (key == "modify"){
			gBody2.modify_channel(_id);
			
		}else if (key == "delete"){
			gBody2.delete_channel(_id);
			
		}else if (key == "exit"){
			gBody2.exit_channel(_id);
			
		}else if (key == "create_share"){
			gBody2.create_channel(null, null, 'Y');
			
		}else if (key == "create_person"){
			gBody2.create_channel(null, null, 'N');
			
		}else if (key == "exit_list"){
			gBody2.draw_exit_info_list("2");
			
		}else if (key == "member"){
			gBody3.draw_channel_members(_id);
			
		}else if (key == "create_folder_share"){
			gBody2.channel_create_folder(null, 'Y');
			
		}else if (key == "create_folder_person"){
			gBody2.channel_create_folder(null, 'N');
			
		}else if (key == "move"){
			gBody2.draw_channel_folder(_id);
		}else if (key == "add_favorite"){
			gBody2.favorite_channel_folder(_id, "add");
		}else if (key == "del_favorite"){
			gBody2.favorite_channel_folder(_id, "del");
		}
		...
		$.ajax({
			...
			success : function(res){
				if (res.result == "OK"){
					...
					gBody2.update_channel_info(null, true);
	...
	"modify_channel" : function(ch_code){
		...
		$.ajax({
			...
			success : function(res){
				if (res.result == "OK"){
					gBody2.create_channel(res.data, null, res.data.ch_share);
...
//		});		
	...
	"channel_user_search" : function(ch_type, str, selectme){
		...
		gsn.requestSearch('', str, function(sel_data){
			...
			for (var i = 0 ; i < sel_data.length; i++){
				...
				if (selectme){	
					gBody2.aleady_select_user_count = 0;
					gBody2.channel_add_user(ch_type, info);
					gBody2.alert_aleady_select_user();					
				}else{
	...
	"channel_list_user" : function(ch_type, obj){
		...
		if (obj.em.toLowerCase() == gap.userinfo.rinfo.em.toLowerCase()){
			...
			gBody2.aleady_select_user_count = 0;
			gBody2.channel_add_user(ch_type, obj);
			gBody2.alert_aleady_select_user();
		}
	},
	...
	"channel_add_user" : function(ch_type, obj){
		...
		//	gap.gAlert(gap.lang.existuser);
			gBody2.aleady_select_user_count += len;
			return false;
		}
		...
		if (ch_type == "C" || ch_type == "CO" || ch_type == "D" || ch_type == "T" || ch_type == "B"){
			...
			html += '	<button class="file_remove_btn" onClick="gBody2.channel_delete_user(this,\'' + ch_type + '\')"></button>';
			html += '</li>';			
			
		}else{
			html += '	<div class="member-profile" id="member_' + id + '"' + (ch_type == "I" ? " style='cursor:pointer' onClick='gBody2.add_folder_member(this)'" : "") + '>';
			if (ch_type != "I"){
				html += '		<button class="ico btn-member-del" onClick="gBody2.channel_delete_user(this,\'' + ch_type + '\')">삭제</button>';
			}
	...
	"alert_aleady_select_user" : function(){
		if (gBody2.aleady_select_user_count == 0){
			//nothing
			
		}else if (gBody2.aleady_select_user_count == 1){
			gap.gAlert(gap.lang.existuser);
			return false;
			
		}else{
			gap.gAlert(gap.lang.existuser + " (" + gBody2.aleady_select_user_count + " " + gap.lang.myung + ")");
			return false;	
		}
	...
	"add_folder_member" : function(obj){
		...
		html += '		<button class="ico btn-member-del" onClick="gBody2.channel_delete_user(this,\'I\')">삭제</button>';
		html += '		<div class="user-result-thumb">' + person_img + '</div>';
		html += '		<dl>';
	...
	"channel_delete_user" : function(obj, ch_type){
		...
		if (ch_type == "F" || ch_type == "I"){
			...
			html += '	<div class="member-profile" id="member_' + id + '" style="cursor:pointer" onClick="gBody2.add_folder_member(this)">';
			html += '		<div class="user-result-thumb">' + person_img + '</div>';
			html += '		<dl>';
			html += '			<dt><span class="status online"></span>' + mem_info.name + '</dt><dd>' + mem_info.dept + '</dd><dd>' + mem_info.company + '</dd>';
			html += '		</dl>';
...
//			html += '			<button class="ico btn-drive-more">더보기</button>';
	...
	//		html += '			<button id="view_mode_gallery" class="ico btn-mode-gallery' + (gBody2.disp_view_mode == "gallery" ? " on" : "") + '"><span>갤러리보기</span></button>';
		...
		var query_str = gBody2.query_str;

		if (gRM.is_box_layer_open){
			$("#drive_data_list").addClass("line2");
		}	
		
		gBody2.draw_drive_title_path();
	...
	/*	var postData = {
				...
				"parent_folder_key" : gBody2.select_folder_code,
				"email" : gap.userinfo.rinfo.em,
				"start" : (gBody2.start_skp - 1).toString(),
				"perpage" : gBody2.per_page,
				"dtype" : gBody.click_filter_image,
				"q_str" : gBody2.query_str,
				"depts" : gap.full_dept_codes()
			};*/
		...
		var postData = {
				...
				"start" : (gBody2.start_skp - 1).toString(),
				"perpage" : gBody2.per_page,
				"dtype" : gBody2.click_filter_image,
				"q_str" : gBody2.query_str
			};
		...
		$.ajax({
			...
			success : function(__res){
				var res = JSON.parse(__res);
				if (res.result == "OK"){
					gBody2.cur_file_count += res.data.datalist.length;
					gBody2.cur_file_total_count = res.data.totalcount;
					gBody2.draw_drive_data_list(page_no, res.data);	
				}else{
					if (gBody2.query_str != "" && res.data == null){
						gBody2.cur_file_count = 0;
						gBody2.cur_file_total_count = 0;
						gBody2.draw_drive_data_list(1, {datalist:[], totalcount:0});	
						return false;
		...
		$("#drive_create_folder_btn").on("click", function(){
			if (gBody2.title_path_name.length == 7){
				// 7 depth면 생성 불가
				gap.gAlert(gap.lang.create_folder_alert);
				return false;
			}
			gBody2.drive_create_folder();
		});
		$("#drive_file_download_btn").on("click", function(){
			gBody2.drive_download_file();
		});
		...
		gBody3.folder_file_upload_event();	
	},
	
	"draw_drive_data_list" : function(page_no, res){
		if (page_no == 1){
			if (gBody2.select_folder_code == "root"){
				if (gBody2.query_str == "" && res.folderlist.length == 0 && res.datalist.length == 0){
					//데이터가 없는 경우
					var html = '<div class="msg-empty"><img src="' + cdbpath + '/img/empty.png" alt="" />' + gap.lang.no_upload_file + '<br />' + gap.lang.upload_use_file + '</div>';
					$("#wrap_drive_data_list").empty();
					$("#wrap_drive_data_list").html(html);
					return false;
				}
				
				if (gBody2.query_str != "" && res.datalist.length == 0){
					//검색된 파일이 없는 경우
					var html = '<div class="msg-empty"><img src="' + cdbpath + '/img/empty.png" alt="" />' + gap.lang.searchnoresult + '</div>';
					$("#wrap_drive_data_list").empty();
					$("#wrap_drive_data_list").html(html);
		...
		if (typeof res.folderlist != "undefined"){
			...
			if (gBody2.select_folder_code != "root"){
				...
				$("#move_parent_folder").on("click", function(){
					if (gBody2.title_path_code.length == 1){
						gBody.cur_folder_info = "";
						gBody.parent_folder_info = "";
						gBody2.select_folder_code = "root";
						gBody2.select_folder_name = "";
						...
						gBody2.start_page = "1";
						gBody2.cur_page = "1";
						gBody3.draw_drive_members(gBody2.select_drive_code);
						gBody2.draw_drive_data(1);
						
					}else{
						var array_cnt = gBody2.title_path_code.length;
						
						gBody.cur_folder_info = gBody.parent_folder_info;
						gBody2.select_folder_code = gBody2.title_path_code[array_cnt - 2];
						gBody2.select_folder_name = gBody2.title_path_name[array_cnt - 2];
						gBody2.title_path_name.pop();
						gBody2.title_path_code.pop();
						gBody2.start_page = "1";
						gBody2.cur_page = "1";
						gBody3.draw_folder_members();
					//	gBody3.draw_drive_members(gBody2.select_drive_code);	// 폴더 클릭 시 드라이버 멤버 대신 폴더 멤버 표시함 - 2021.11.16
			...
			for (var i = 0;  i < res.folderlist.length; i++){
				...
				$("#folder_" + folder_id).bind("click", folder_info, function(event){
					...
					gBody.parent_folder_info = gBody.cur_folder_info;
					gBody.cur_folder_info = event.data;
					gBody2.select_folder_code = folder_id;
					gBody2.select_folder_name = (gBody2.disp_view_mode == "list" ? $("#" + select_id + " dl dt strong").text() : $("#" + folder_id + " dl dd strong").text());
					gBody2.title_path_code.push(gBody2.select_folder_code);
					gBody2.title_path_name.push(gBody2.select_folder_name);
				//	gBody3.draw_members(gBody.cur_folder_info);
					gBody3.draw_folder_members();
				//	gBody3.draw_drive_members(gBody2.select_drive_code);	// 폴더 클릭 시 드라이버 멤버 대신 폴더 멤버 표시함 - 2021.11.16
					gBody2.draw_drive_data(1);
				});
				...
				$("#member_" + folder_id).bind("click", folder_info.member, function(event){
					gBody2.show_folder_member($(this), event.data);
				});
		...
		//	setTop : ($("#wrap_drive_data_list").height()) + "px",
			callbacks : {
				...
				onTotalScroll: function(){
					if (gBody2.disp_view_mode != "list"){
						page_no++;
						gBody2.add_drive_data_list(page_no);
					}
				},
				onTotalScrollOffset: 100,
				alwaysTriggerOffsets:true
	...
	"draw_drive_title_path" : function(){
		//<a href="#">개인보관함</a><a href="#">디자인팀</a><span>2021년 트렌드</span>
		var html = '';
		if (gBody2.select_folder_code == "root"){
			html += '<span>' + gBody2.select_drive_name + '</span>';
		}else{
			html += '<a style="cursor:pointer" id="title_' + gBody2.select_drive_code + '">' + gBody2.select_drive_name + '</a>';
		}
	//	$("#drive_title_path").html(html + gBody3.expand_collapse_btn());
		$("#drive_title_path").html(html);
		$("#title_" + gBody2.select_drive_code).on("click", function(){
			gBody2.select_folder_code = "root";
			gBody2.select_folder_name = "";
			gBody2.title_path_name = [];
			gBody2.title_path_code = [];
		...
		for (var i = 0; i < gBody2.title_path_name.length; i++){
			var _html = '';
			if (i == (gBody2.title_path_name.length - 1)){
				_html += '<span>' + gBody2.title_path_name[i] + '</span>';
			}else{
				_html += '<a style="cursor:pointer" id="title_' + gBody2.title_path_code[i] + '">' + gBody2.title_path_name[i] + '</a>';
			}
			$("#drive_title_path").append(_html);
			$("#title_" + gBody2.title_path_code[i]).on("click", function(){
				var select_id = $(this).attr("id");
				gBody2.select_folder_code = select_id.split("title_")[1];
				gBody2.select_folder_name = $(this).text();
				gBody2.title_path_name = [];
				gBody2.title_path_code = [];
				gBody2.start_page = "1";
				gBody2.cur_page = "1";
				$("#drive_title_path a").each(function(idx) {
					if (idx != 0){
						gBody2.title_path_name.push($(this).text());
						gBody2.title_path_code.push($(this).attr("id").split("title_")[1]);
	...
	"drive_create_folder" : function(obj){
		var is_update = (obj != undefined ? true : false);
		var _drive_info = $.map(gBody.cur_drive_list_info, function(ret, key) {
			if (ret.ch_code == gBody2.select_drive_code){
				return ret;
			}
		});
		var is_folder_share = (obj != undefined ? obj.folder_share : _drive_info[0].ch_share);
		var html = gBody2.folder_layer_html(is_update, is_folder_share);
		...
		if (is_update){
			...
			if (obj.folder_share == "Y"){
				...
				if (obj.parent_folder_key == "root"){
					// 드라이브 멤버
					_parent_member = $.map(gBody.cur_drive_list_info, function(ret, key) {
				...
				for (var i = 0; i < obj.member.length; i++){
					...
					// 폴더 멤버 표시
					gBody2.channel_add_user("F", obj.member[i]);
				}
				
				for (var j = 0; j < _parent_member.length; j++){
					// parent 멤버 중 초대 가능 멤버 표시
					gBody2.channel_add_user("I", _parent_member[j]);
				}
			}
			/////////////////////////////////////////////////////////////
			...
			if (gBody2.select_folder_code == "root"){
				if (_drive_info[0].owner.ky != gap.userinfo.rinfo.ky){
					gBody2.channel_add_user("I", _drive_info[0].owner);
				}
				if (_drive_info[0].member){
					...
					for (var i = 0; i < _member.length; i++){
						if (_member[i].ky != gap.userinfo.rinfo.ky){
							gBody2.channel_add_user("I", _member[i]);
						}
					}
				}
				
			}else{
				if (gBody.cur_folder_info.owner.ky != gap.userinfo.rinfo.ky){
					gBody2.channel_add_user("I", gBody.cur_folder_info.owner);
				}
				if (gBody.cur_folder_info.member){
					var _member = gBody.cur_folder_info.member;
					for (var i = 0; i < _member.length; i++){
						if (_member[i].ky != gap.userinfo.rinfo.ky){
							gBody2.channel_add_user("I", _member[i]);
						}
			...
			/////////////////////////////////////////////////////////////
		/*	
		 * 폴더에 권한을 부여하지 않음 - 2021.10.07	
			var surl = gap.channelserver + "/memberlist.km";
			var postData = JSON.stringify({
					"drivekey" : gBody2.select_drive_code,
					"folderlist" : gBody2.title_path_code.join("-spl-"),
					"email" : gap.userinfo.rinfo.em
				});			
				...
				success : function(res){
					if (res.result == "OK"){
						for (var i = 0; i < res.data.members.length; i++){
							gBody2.channel_add_user("F", res.data.members[i]);
						}
					}
				},
				error : function(e){
	...
	//	var drive_key = $(".drive-code.on").attr("id").replace("main_", "");
		var drive_key = (is_update ? obj.drive_key : gBody2.select_drive_code);
		var parent_folder_key = (is_update ? obj.parent_folder_key : gBody2.select_folder_code);
		var folder_depth = (is_update ? obj.depth : (gBody2.title_path_code.length + 1).toString());
		...
		$("#input_folder_user").keydown(function(evt){
			if (evt.keyCode == 13){
				...
				gBody2.channel_user_search("F", $("#input_folder_user").val() );
			}else{
				$("#same_name_list").empty();
				$("#same_name_list").hide();
			}
			...
			window.ORG.show(
					...
					}, 
					{
						...
						setItems:function(items) { /* 반환되는 Items */
							...
							for (var i = 0; i < items.length; i++){
								var _res = gap.convert_org_data(items[i]);
								gBody2.channel_add_user('F', _res);
							}
							gBody2.alert_aleady_select_user();
						}
					});
		...
		/*	var is_folder_share = "";
			...
			if (is_update){
				is_folder_share = obj.folder_share;
				...
				is_folder_share = _drive_info.ch_share;
			}*/
			...
			if (is_folder_share == "N"){
				...
				/*
				 * 위에 코드로 대체 - 2021.11.16
				readers = _drive_info.readers;
				memeber_list.push(_drive_info.owner);
				for (var k = 0; k < _drive_info.member.length; k++){
					memeber_list.push(_drive_info.member[k]);
				}

				for (var i = 0; i < memeber_list.length; i++){
					if (memeber_list[i].ky != gap.userinfo.rinfo.ky){
						user_list.push(memeber_list[i]);
					}
				}*/
					...
					if (res.result == "OK"){
						...
						if (is_update){
							...
							//	html += '		<span class="ico ico-folder' + (member_count != 0 ? "-share" : "") + '"></span>';
								...
								/*if (member_count != 0){
									html += '	<div class="share-member share-folder-member" id="member_' + folder_id + '">';
									html += '		<div class=""><button class="btn-view-member"><span class="ico ico-share-member"></span>' + member_count + gap.lang.myung + '</button></div>';
									html += '	</div>';				
								}*/
							...
							//	html += '	<div class="thm" id="folder_' + folder_id + '"><span class="ico ico ico-folder' + (member_count != 0 ? "-share" : "") + '-b"></span></div>';
								...
								/*if (member_count != 0){
									html += '	<div class="share-member" id="member_' + folder_id + '">';
									html += '		<div class=""><button class="btn-view-member"><span class="ico ico-share-member"></span>' + member_count + gap.lang.myung + '</button></div>';
									html += '	</div>';
								}*/
							}
						...
						$("#folder_" + folder_id).on("click", function(){
							...
							gBody.parent_folder_info = gBody.cur_folder_info;
							gBody.cur_folder_info = _info;
							
							gBody2.select_folder_code = folder_id;
							gBody2.select_folder_name = $("#" + (gBody2.disp_view_mode == "list" ? select_id : folder_id) + " dl dt strong").text();
							gBody2.title_path_code.push(gBody2.select_folder_code);
							gBody2.title_path_name.push(gBody2.select_folder_name);
							gBody3.draw_folder_members();
							gBody2.draw_drive_data(1);
						...
						if (member_count != 0){
							for (var i = 0; i < user_list.length; i++){
								...
								var _noti = new Object();
								_noti.type = "drive_member";								
								_noti.p_code = (is_update ? obj._id.$oid : res.folder_id);
								_noti.p_name = folder_name;
								_noti.title = "";
								_noti.sender = user_list[i].ky;
								_wsocket.send_drive_msg(_noti);
							}
						}
						*/
						...
						// 드라이브 title path 변경
						if (!gBody3.check_top_menu_new()){
							if (gBody2.select_folder_code == folder_id){
						...
						// 오른쪽 멤버 업데이트
						if (gBody2.select_folder_code == folder_id){
							gBody2.update_folder_info(folder_id);
...
/*		$("#folder_add_member_list, #folder_member_list").mCustomScrollbar({
			theme:"dark",
			autoExpandScrollbar: false,
			scrollButtons:{
				enable: false
	...
	"context_menu_call_folder_mng" : function(key, options, _id){
		...
		if (key == "modify"){
			gBody2.drive_modify_folder(_id);
			
		}else if (key == "delete"){
			gBody2.drive_delete_folder(_id);
		}
	},
...
//		});
	...
	"drive_modify_folder" : function(folder_id){
		...
		$.ajax({
			...
			success : function(res){
				if (res.result == "OK"){
					gBody2.drive_create_folder(res.data);
				}else{
					gap.gAlert(gap.lang.errormsg);
					return false;					
				}
	...
	"favorite_file_menu_content" : function(is_preview){
		...
		if (is_preview){
			items["favorite_preview"] = {name : gap.lang.preview}
		}
		items["download"] = {name : gap.lang.download}
	//	items["copyurl"] = {name : gap.lang.copyurl};
		items["sep01"] = "-------------";
	...
	"context_menu_call_file_mng" : function(key, options, _id, _fserver, _fname, _md5, _furl, is_search, owner, upload_path, ftype){
		...
		if (key == "favorite"){
			...
			gBody2.add_favorite_file(_id, _md5, "1");
			
		}else if (key == "files_favorite"){
			gBody2.add_favorite_file(_id, _md5, "2");
			
		}else if (key == "download"){
			gap.file_download_normal(_furl, _fname);
			
		}else if (key == "move"){
			gBody2.drive_move_file(_id);
			
		}else if (key == "delete"){
			gBody2.drive_delete_select_item(_id);
			
		}else if (key == "favorite_delete"){
			gBody2.delete_favorite_file(_id);
			
		}else if (key == "files_delete"){
			
			if (_id.split("_").length > 1){
				//댓글의 파일을 삭제하는 경우     )" : function(channel_id, rid, md5, ftype, obj){
				gBody3.del_reply_att(_id.split("_")[0], _id, _md5, ftype, "reply");
			}else{
				gBody2.delete_files_file(_id, _fname, _md5, options);
			}
			...
			if (show_video){
				...
				gBody3.file_convert(_fserver, _fname, _md5, _id, "1", call_from_search);				
			}
			...
			if (show_video){
				...
				gBody3.file_convert(_fserver, _fname, _md5, _id, "3", call_from_search);			
			}
			...
			if (show_video){
				...
				if (_id.split("_").length > 1){
					...
					gBody3.file_convert(_fserver, _fname, _md5, _id, ty, ftype, owner, upload_path);	
				}else{
					gBody3.file_convert(_fserver, _fname, _md5, _id, ty, call_from_search);	
				}
				
			}
			
		}else if (key == "copyurl"){
			gBody2.copy_file_url(_furl);
			
		}else if (key == "regdrive"){
			gBody2.files_reg_drive_select_item(_id + "-spl-" + _md5);
				...
				for (var i = 0; i < res.length; i++){
					...
					$("#move_" + data.ch_code).on("click", function(e){
						if (e.target.className == "ico-fold"){
							...
							gBody2.draw_sub_folder(drive_code, folder_code, 24, true);
		...
		$("#run_move_file").on("click", function(){
			...
			$("#drive_tree div").each(function(idx){
				if ($(this).hasClass("on")){
					...
					full_path_code = gBody2.get_move_fullpath($(this), $(this).attr("id").replace("move_", ""));
				}
			});
			if (full_path_code == ""){
...
/*			if (selected_id == gBody2.select_folder_code){
				gap.gAlert("동일한 위치로 이동할 수 없습니다.");
				return false;
			}*/
			...
			$.ajax({
				...
				success : function(res){
					if (res.result == "OK"){
						/*if (gBody2.select_left_menu != ""){
							gBody2.add_main_data_list(gBody2.cur_page);
						...
						if (gBody2.disp_view_mode == "list"){
							var key1 = (gBody2.total_file_count - file_check.join().split(",").length);
							var key2 = (gBody2.cur_page * gBody2.per_page - gBody2.per_page);
							if (key1 > key2){
								if (gBody2.select_left_menu == ""){	// 개별 드라이브를 클릭한 경우
									gBody2.draw_drive_data(gBody2.cur_page);
									
								}else{	// 전체/내가올린/공유된/즐겨찾기 클 클릭한 경우
									gBody2.draw_main_data(gBody2.cur_page);
								}
								...
								if (gBody2.select_left_menu == ""){	// 개별 드라이브를 클릭한 경우
									gBody2.draw_drive_data(page);
									
								}else{	// 전체/내가올린/공유된/즐겨찾기 클 클릭한 경우
									gBody2.draw_main_data(page);
								}
							}
					...
					//	_html += '			<span class="ico-fold"></span><span class="ico ico-folder' + (folder_info.folder_share == "Y" ? "-share-s" : "-s") + '"></span>' + folder_name;
						...
						$("#move_" + folder_id).on("click", function(e){
							if (e.target.className == "ico-fold"){
								...
								$("#" + $(this).attr("id") + " .ico-fold").addClass("on");
								gBody2.draw_sub_folder(drive_code, folder_code, padding_left_set, false);
	...
	"get_move_fullpath" : function(_selector, _path){
		if (_selector.attr("data") == "root"){
			...
			if (_selector.parent().parent().siblings("div").attr("data") == "root"){
				...
				return gBody2.get_move_fullpath(_ele, _fullpath);
			}		
		}
	},
				...
				confirm : {
					...
					action : function(){
						...
						var postData = {
								...
								"file_item" : file_check.join("-spl-"),
								"drive_code" : gBody2.select_drive_code,
								"folder_depth" : gBody2.title_path_code
							};			
							...
							success : function(res){
								if (res.result == "OK"){
									if (gBody2.disp_view_mode == "list"){
										...
										var key2 = (gBody2.cur_page * gBody2.per_page - gBody2.per_page);
										if (key1 > key2){
											gBody2.draw_favorite_data(gBody2.cur_page);
										}else{
											var page = gBody2.cur_page - 1;
											if (page <= 0){
												page = 1;
											}
											gBody2.draw_favorite_data(page);
										}
...
//		});
					...
					action : function(){
						...
						$.ajax({
							...
							success : function(res){
								if (res.result == "OK"){
									if (gBody2.disp_view_mode == "list"){
										var key1 = (gBody2.total_file_count - fid_array.join().split(",").length);
										var key2 = (gBody2.cur_page * gBody2.per_page - gBody2.per_page);
										if (key1 > key2){
											gBody2.draw_files(gBody2.cur_page);
										}else{
			...
			success : function(res){
				if (res.result == "OK"){
					if (isNaN(res.data.count) || res.data.count == 0){
		                ...
		                if (call_from_search){
		                	gRM.show_convert_search_file_info(fname, md5, res.data.count);
		                	
		                }else{
		                	gRM.show_convert_file_info(fname, md5, res.data.count);
		                }
					}
		...
		$.ajax({
			...
			success : function(__res){
				...
				if (res.result == "OK"){
					...
					gBody2.cur_file_total_count = res.data.totalcount;
					gBody2.draw_favorite_data_list(page_no, res.data);	
					
				}else{
					if (gBody2.query_str != "" && res.data == null){
						gBody2.cur_file_count = 0;
						gBody2.cur_file_total_count = 0;
						gBody2.draw_favorite_data_list(1, {data:[], totalcount:0});	
						return false;
	...
	"draw_favorite_data_list" : function(page_no, res){
		if (gBody2.query_str == "" && res.data.length == 0){
			//데이터가 없는 경우
			var html = '<div class="msg-empty"><img src="' + cdbpath + '/img/empty.png" alt="" />' + gap.lang.no_favorite_file + '</div>';
		...
		for (var k = 0;  k < res.data.length; k++){
			...
			if (gBody2.disp_view_mode == "list"){
				...
				file_html += '			<span class="ico ico-file ' + icon_kind + '"></span>';
				file_html += '			<dl>';
				file_html += '				<dt><strong>' + disp_file_name + '</strong> <span>(' + gap.file_size_setting(file_info.size.$numberLong) + ')</span></dt>';
				file_html += '				<dd><span>' + user_info.name + gap.lang.hoching  + '</span><em>' + disp_date + ' ' + disp_time + '</em></dd>';
				file_html += '			</dl>';
				file_html += '		</a>';
		...
		$(".ico.btn-more.favorite-file-menu").on("click", function(){
			$.contextMenu({
				...
				build : function($trigger, options){
					...
					options.handleObj.data.zIndex = parseInt(gap.maxZindex()) + 1;
					return {
						items: gBody2.favorite_file_menu_content(is_preview)
					}
				}
			});
		});	
		...
		//	setTop : ($("#wrap_favorite_data_list").height()) + "px",
			callbacks : {
				...
				onTotalScroll: function(){
					if (gBody2.disp_view_mode != "list"){
						page_no++;
						gBody2.add_favorite_data_list(page_no);
					}
				},
				onTotalScrollOffset: 100,
				alwaysTriggerOffsets:true
		...
		$.ajax({
			...
			success : function(__res){
				...
				if (res.result == "OK"){
					gBody2.cur_file_count += res.data.data.length;
					gBody2.cur_file_total_count = res.data.totalcount;
					gBody2.draw_files_data_list(page_no, res.data);
					
				}else{
					if (gBody2.query_str != "" && res.data == null){
						gBody2.cur_file_count = 0;
						gBody2.cur_file_total_count = 0;
						gBody2.draw_files_data_list(1, {data:[], totalcount:0});	
						return false;
			...
			//	gRM.box_layer_close();
				...
				$.ajax({
					...
					success : function(res){
						if (res.result == "OK"){
							var file_info = res.data;
							//gBody2.draw_file_detail_info(file_info, "2")
		...
		$(".ico.btn-more.files-file-menu").on("click", function(){
			...
			$.contextMenu({
				...
				callback : function(key, options){
					...
					gBody2.context_menu_call_file_mng(key, options, f_id, f_server, f_name, f_md5, f_url, false,  f_owner, f_upload_path, f_type);
				},
				events : {
					hide: function (options) {
						$(this).removeClass("on");
				...
				build : function($trigger, options){
					...
					options.handleObj.data.zIndex = parseInt(gap.maxZindex()) + 1;
					return {
						items: gBody2.files_file_menu_content(is_preview, fowner)
					}
				}
			});
		});	
		...
		//	setTop : ($("#wrap_favorite_data_list").height()) + "px",
			callbacks : {
				...
				onTotalScroll: function(){
					if (gBody2.disp_view_mode != "list"){
						page_no++;
						gBody2.add_files_data_list(page_no);
					}
				},
				onTotalScrollOffset: 100,
				alwaysTriggerOffsets:true
	...
	"draw_file_detail_info" : function(info, ty){
		...
		$("#channel_right").show();		
		gRM.box_layer_close(); //우측 프레임을 연다
	...
	//	if (gBody3.check_preview_file(filename) && info.thumbOK == "T"){
		if (gBody3.check_preview_file(filename) || ext == "movie" || ext == "img"){
	...
	/*	html += "	<div class='file-path'>";
		html += gap.lang.file_path + " : " + gBody2.get_file_path(info.drive_code, info.folder_code);
		html += "	</div>";*/
		...
		if ( (typeof(info.meta) != "undefined") && (info.meta != "")){		
			...
			if (typeof(info.drive_code) != "undefined"){
				...
				html += "</tr>"
				html += "<tr>"
				html += "	<td colspan='2' style='padding-left:20px;'><strong>" + gBody2.get_file_path(info.drive_code, info.folder_code) + "</strong></td>"
				html += "</tr>"
			}
...
//				if (icon_kind == "img"){
//					if (ty == "2"){
//						var furl = fserver + "/FDownload.do?id=" + id + "&md5=" + md5 + "&ty=" + ty+"&ky="+gap.userinfo.rinfo.ky;
//						
//					}else{
//						var furl = fserver + "/FDownload.do?id=" + id + "&ty=" + ty + "&ky="+gap.userinfo.rinfo.ky;
//					}
//					
//					gap.image_gallery = new Array();  //변수 초기화 해준다.
//					gap.image_gallery_current = 1;
...
//					
//				}else{
				//	gBody2.file_convert(fserver, fname, md5, id, ty);	
					var pt = "";
					gBody3.file_convert(fserver, fname, md5, id, ty, file_type ,info.email, upload_path);	
...
//				}
	...
	"_______draw_file_detail_info_______" : function(info, ty){
		gBody3.select_file_info = info;
		...
		var dis_date = gap.change_date_localTime_full2(info.GMT);
		var _info = (ty == "2" ? info.info : info);
		var filename = _info.filename;
		var size = gap.file_size_setting(parseInt(typeof _info.file_size == "undefined" ? _info.size.$numberLong : _info.file_size.$numberLong));
		...
		if (ty == "3"){
			...
			var name = info.owner.nm;
			var email = info.owner.ky;
			if (gap.userinfo.rinfo.el != info.owner.el){
				name = info.owner.enm;
			}
		}
		
		var html = "";
		html += "<div class='detail-info' id='detail_info_dis' style='height:100%; margin-top:5px'>";
	//	if (gBody3.check_preview_file(filename)){
		if (_info.thumbOK == "T"){
			html += "<button class='ico btn-file-view' id='detail_file_info_preview' style='width:16px;height:16px;position:absolute;top:20px;right:45px;background-position: -652px -20px;'>미리보기</button>";			
		}
		html += "<button class='ico btn-right-close' id='detail_file_info_close'>닫기</button>";
		...
		if ( (typeof(_info.meta) != "undefined") && (_info.meta != "")){
			html += "	<button class='ico btn-detail-open'>펼치기</button>";
			html += "	<a>" + filename + "<span>&nbsp;(" + size + ")</span></a>";
			html += "	<div class='file-info-detail'>";
			html += "		<table>";
			...
			var metainfo = JSON.parse(_info.meta);
			var file_type = gap.is_file_type(filename);
			metainfo.size = size;
			html += gap.draw_file_info(metainfo, file_type);	
...
//		});
	...
	"context_menu_call_channel_files_mng" : function(key, options, _id, _fserver, _fname, _md5, _furl){
		if (key == "favorite"){
			gBody2.channel_favorite_file(_id, _md5);
			
		}else if (key == "preview"){
			gBody2.file_convert(_fserver, _fname, _md5, _id, "2");
...
//			});
	...
	"drive_file_upload" : function(){
		gBody2.init_select_drive_folder();
		
		var html = gBody2.drive_fileupload_layer_html();
		...
		$.ajax({
			...
			success : function(res){
				...
				$("#drive_option_list").on("change", function(){
					gBody2.select_drive_code = $(this).val();
					gBody2.select_drive_name = $('option:selected',this).text();
					gBody2.select_folder_code = "root";
					gBody2.select_folder_name = "";
					gBody2.title_path_code = [];
					gBody2.title_path_name = [];
					gBody2.drive_folder_file_list(1);
		...
		$("#drive_fileupload_btn").on("click", function(){
			...
			$.ajax({
				...
				success : function(ress){
					var res = JSON.parse(ress);
					if (res.result == "OK"){
					//	gBody2.draw_folder_file_list(page_no, res.data);
						gBody2.init_select_drive_folder();
						gap.close_layer('drive_fileupload_layer');
	...
	"drive_folder_file_list" : function(page_no){
		if (page_no == 1){
			...
			if (gBody2.cur_file_count == gBody2.cur_file_total_count){
				return false;
			}				
		}
	...
	"draw_folder_file_list" : function(page_no, res){
		gBody2.draw_selected_folder_path();
		 ...
		 * if (typeof res.folderlist != "undefined"){
			...
			//	folder_html += '	<span class="ico ico-folder' + (folder_info.folder_share == "Y" ? "-share" : "") + '"></span>';
				...
				$("#fl_" + folder_id).on("click", function(){
					gBody2.select_folder_code = $(this).attr("id").replace("fl_", "");
					gBody2.select_folder_name = $(this).find(".attach-name span").text();
					gBody2.title_path_code.push(gBody2.select_folder_code);
					gBody2.title_path_name.push(gBody2.select_folder_name);
		...
		$("#wrap_upload_folder_file_list").mCustomScrollbar({
			...
			callbacks : {
				onTotalScroll: function(){
					page_no++;
					gBody2.drive_folder_file_list(page_no);
				},
				onTotalScrollOffset:100,
				alwaysTriggerOffsets:true
			}
	...
	"draw_selected_folder_path" : function(){
		var folder_code_list = gBody2.title_path_code;
		var folder_name_list = gBody2.title_path_name;
		
		if (folder_code_list.length > 0){
			var html = '<span ty="drive" data-id="' + gBody2.select_drive_code + '">' + gBody2.select_drive_name + '</span>';
			...
			$("#selected_path_layer span").on("click", function(){
				...
				if (ty == "drive"){
					...
					gBody2.select_folder_code = $(this).attr("data-id");
					gBody2.select_folder_name = $(this).text();
					gBody2.title_path_code = tmp_code_list;
					gBody2.title_path_name = tmp_name_list;
					gBody2.drive_folder_file_list(1);
				}
			});
		}else{
			var html = '<strong>' + gBody2.select_drive_name + '</strong>';
			$("#selected_path_layer").html(html);
		}
	},
...
//		html += '	<button><strong>' + gap.lang.OK + '</strong></button>';
				...
				if (res.result == "OK"){
					for (var i = 0; i < res.data.data.length; i++){
						...
						$("#enter_" + _info.ch_code).bind("click", _info, function(event){
							gBody2.enter_info(event.data, ty);
						});
			...
			success : function(res){
				if (res.result == "OK"){
					...
					if (_ty == "1"){
						...
						gBody2.draw_main_sub_folder(res_list);
						...
						if (_info.ch_name.length > gBody2.menu_length){
							dischname = _info.ch_name.substring(0,gBody2.menu_length) + "...";
						}
	...
	"initialize_page" : function(){
		var alldocuments = gBody2.total_file_count;
		if (alldocuments % gBody2.per_page > 0 & alldocuments % gBody2.per_page < gBody2.per_page/2 ){
			gBody2.all_page = Number(Math.round(alldocuments/gBody2.per_page)) + 1
		}else{
			gBody2.all_page = Number(Math.round(alldocuments/gBody2.per_page))
		}	

		if (gBody2.start_page % gBody2.per_page > 0 & gBody2.start_page % gBody2.per_page < gBody2.per_page/2 ){
			gBody2.cur_page = Number(Math.round(gBody2.start_page/gBody2.per_page)) + 1
		}else{
			gBody2.cur_page = Number(Math.round(gBody2.start_page/gBody2.per_page))
		}

		gBody2.initialize_navigator();		
	},
	
	"initialize_navigator" : function(){
		var alldocuments = gBody2.total_file_count;

		if (gBody2.total_page_count == 0){
			gBody2.total_page_count = 1;
		}
		...
		if (alldocuments != 0) {
			if (gBody2.total_page_count % 10 > 0 & gBody2.total_page_count % 10 < 5 ){
				var all_frame = Number(Math.round(gBody2.total_page_count / 10)) + 1
			}else{
				var all_frame = Number(Math.round(gBody2.total_page_count / 10))	
			}

			if (gBody2.cur_page % 10 > 0 & gBody2.cur_page % 10 < 5 ){
				var c_frame = Number(Math.round(gBody2.cur_page / 10)) + 1
			}else{
				var c_frame = Number(Math.round(gBody2.cur_page / 10))
			}
			...
			if (c_frame == 1){
				...
				nav[0] = '<li class="p-prev" onclick="gBody2.goto_page(' + ((((c_frame-1) * 10) - 1)*gBody2.per_page + 1) + ',' + ((c_frame - 1) * 10) + ');"><span>이전</span></li>';
			}			
			...
			for (var i = start_page; i < start_page + 10; i++){
				if (i == gBody2.cur_page){
					...
					if (i == '1'){
						nav[pIndex] = '<li onclick="gBody2.goto_page(' + (((i-1) * gBody2.per_page) + 1 ) + ', ' + i + ', this)">' + i + '</li>';
					}else{
						if (i % 10 == '1' ){
							nav[pIndex] = '<li onclick="gBody2.goto_page(' + (((i-1) * gBody2.per_page) + 1 ) + ', ' + i + ', this)">' + i + '</li>';
						}else{
			...
			if (c_frame < all_frame){
				nav[nav.length] = '<li class="p-next" onclick="gBody2.goto_page(' + ((c_frame * gBody2.per_page * 10) + 1) + ',' + ((c_frame * 10) + 1) + ');"><span>다음</span></li>';
			}
			var nav_html = '';

			if (c_frame != 1 ){
				nav_html = '<li class="p-first" onclick="gBody2.goto_page(1,1);"><span>처음</span></li>';
			}		    
			...
			if (c_frame < all_frame){
				nav_html += '<li class="p-last" onclick="gBody2.goto_page(' + ((gBody2.all_page - 1) * gBody2.per_page + 1) + ',' + gBody2.all_page + ')"><span>마지막</span></li>';
			}
	...
	"goto_page" : function(idx, page_num){
		if (gBody2.total_file_count < idx) {
			gBody2.start_page = idx - 10;
			if ( gBody2.start_page < 1 ) {
				return;
			}
		}else{
			gBody2.start_page = idx;
		}
		cur_page = page_num;
		if (gBody3.select_files_tab){
			// Files 탭이 클리된 경우
			gBody2.add_files_data_list(page_num);
			
		}else if (gBody3.cur_opt == "favoritecontent"){
			// 즐겨찾기
			gBody2.add_favorite_data_list(page_num);
			
		}else{
			// 드라이브
			if (gap.cur_window == "drive" && gBody2.select_left_menu != ""){
				// 전체 / 내가올린/ 공유된 / 즐겨찾기
				gBody2.add_main_data_list(page_num);
				
			}else{
				gBody2.add_drive_data_list(page_num);
			}
		}
	},
	...
	"draw_empty_search_content" : function(){
		...
		if (is_empty){
			...
			empty_html += '	<img src="' + cdbpath + '/img/doc.png" alt="" />';
	...
	"box_search" : function(query, category, page_no){
		...
		gBody2.box_search_data(query, category, page_no);
		...
		$("#portal_tab li").on("click", function(e){
			...
			gBody2.box_search_data(gBody2.ts_query, ts_categry, 1);
		});
...
/*		_html += '		<div class="drive-btns">';
	...
	"box_search_data" : function(query, category, page_no){
		...
		if (page_no == 1){
			gBody2.ts_query = query;
			gBody2.ts_category = category;
			gBody2.cur_file_count = 0;
			gBody2.cur_file_total_count = 0;
			$("#box_search_list").empty();
		}
		gBody2.draw_empty_search_content();
		gBody2.start_skp = (parseInt(gBody2.ts_per_page) * (parseInt(page_no))) - (parseInt(gBody2.ts_per_page) - 1);
		
		var surl = gap.channelserver + "/msearch.km";
	/*	var postData = JSON.stringify({
				"q_str" : gBody2.ts_query.toLowerCase(),
				"email" : gap.search_cur_em(),
				"channels" : gBody2.my_channel_info,
				"drives" : gBody2.my_drive_info,
				"folders" : gBody2.my_folder_info,
				"category" : gBody2.ts_category,
				"gubun" : gTop.select_search_type,
				"start" : (gBody2.start_skp - 1).toString(),
				"perpage" : gBody2.ts_per_page
			});	*/
		...
		$.ajax({
			...
			success : function(res){
				if (res.result == "OK"){
					gBody2.cur_file_count += res.data.hits.hits.length;
					gBody2.cur_file_total_count = res.data.hits.total;
...
//				gBody2.file_convert(fserver, fname, md5, id, ty, true);	
			...
			/*if (thumbOK == "T"){
				...
				$.ajax({
					...
					success : function(res){
						if (res.result == "OK"){
							$("#ext_body").removeClass("channel view-info chat-area");
							$("#ext_body").addClass("view-info");
							
							var file_info = res.data;
							gBody2.draw_search_file_detail_info(file_info, ty)
			...
			if (e.target.className == "ico btn-more file-menu"){
				$.contextMenu({
					...
					callback : function(key, options){
						...
						var f_md5 = $selector.attr("md5");
						var f_url = $selector.attr("furl");
						gBody2.ts_select_id = f_id;
						gBody2.context_menu_call_file_mng(key, options, f_id, f_server, f_name, f_md5, f_url, true);
					},
					events : {
						hide: function (options) {
							$(this).removeClass("on");
				...
				$.contextMenu({
					...
					callback : function(key, options){
						...
						var f_md5 = $selector.attr("md5");
						var f_url = $selector.attr("furl");
						gBody2.ts_select_id = $selector.attr("id");;
						gBody2.context_menu_call_file_mng(key, options, f_id, f_server, f_name, f_md5, f_url, true);
					},
					events : {
						hide: function (options) {
				...
				gBody2.ts_select_id = $(this).attr("id");
				gBody3.show_search_content($(this))
			}
		});
	},
	...
	"event_init_ts_search_file" : function(){
		...
		$(".chat-attach .btn-file-view").on("click", function(e){
			...
			//	gBody2.channel_file_convert(fserver, filename, md5, id, "2", ft, email, upload_path);
				//AP-ON 용
			//	gBody2.channel_file(fserver, filename, md5, id, "2", ft, email, upload_path);
				//대상용
				gBody3.file_convert(fserver, filename, md5, id, "2", ft, email, upload_path);
	...
	"channel_create_folder" : function(obj, is_share){
		...
		var html = gBody2.channel_folder_layer_html(is_update);
	...
	/*	var postData = {
		...
		$.ajax({
			...
			success : function(res){
				if (res.result == "OK"){
					gBody2.channel_create_folder(res.data, res.data.share);
...
//		});
		...
		//	 helper: 'clone',
			 ...
			 helper: function (e) { 
				...
				if (sid.indexOf("mCSB_") > -1){
					...
					//gBody2 에서 channel_menu_content 함수에서 gBody2.channel_menu_content_click_id 값을 설정함
					xtxt = $("#"+ gBody2.channel_menu_content_click_id).parent().find("em").text();
				}else{
				...
				try{					
			 	...
			 	//	if ($(droppable).parent().find("#favorite_channel_list").length > 0){
			 		if ($(droppable).parent().attr("class") == "favorit"){
			 			//즐겨찾기 영역에 드롭한 경우이다.
			 			gBody2.favorite_channel_folder(dragid, "add");
			 		}else{
		...
		$("#share_channel_list .folder-code, #person_channel_list .folder-code").on("click", function(e){
			if (e.target.className == "ico btn-more ch-folder-mng"){
				$.contextMenu({
					...
					trigger : "left",
					callback : function(key, options){					
						//gBody2.context_menu_call_channel_folder_mng(key, options, $(this).parent().attr("id"));
						gBody2.context_menu_call_channel_folder_mng(key, options, $(this).parent().parent().attr("id"));
					},
					events : {
						hide: function (options) {
							$(this).removeClass("on");
					...
					//	var folderkey = $($trigger).parent().attr("id").replace("div_", "");
						...
						return {
							items: gBody2.channel_folder_menu_content(owner, is_channel_share)
						}
					}
				});
	...
	"context_menu_call_channel_folder_mng" : function(key, options, _id){
		...
		if (key == "modify_folder"){
			gBody2.channel_modify_folder(_id);
			
		}else if (key == "delete_folder"){
			gBody2.channel_delete_folder(_id);
			
		}else if (key == "create_share_channel"){
			gBody2.create_channel(null, _id, 'Y');
			
		}else if (key == "create_person_channel"){
			gBody2.create_channel(null, _id, 'N');
		}
	},
...
//		html += '	<button><strong>' + gap.lang.OK + '</strong></button>';
		...
		$.ajax({
			...
			success : function(res){
				...
				$(".btn-entry").on("click", function(){
					...
					fid = (fid == "root" ? "" : fid);
					gBody2.move_channel_to_folder(cid, fid);
				});
			},
			error : function(e){
				gap.error_alert();
	...
	"draw_drive_left" : function(res){
		...
		if (typeof(res.drive) != "undefined"){
			...
			// 드라이브 하위 폴더 그리기
			gBody2.draw_main_sub_folder(res.folder);		
		}
	...
	"show_all_files" : function(user_obj){
		gBody2.all_files_selected_member = "";
	...
	//	$("#common_work_layer").html(html);
		...
		gBody2.all_files_selected_tab = "workroom_files";
		gBody2.all_files_selected_member = user_obj;
		
		// 기본 호출 목록
		gBody2.get_channel_all_files(1);
		
		// 이벤트 처리
		gBody2.event_all_files($layer);
	},
	...
	"event_all_files" : function($layer){
		...
		$layer.find('.work-tab li').off().on('click', function(){
			...
			// 선택된 탭
			gBody2.all_files_selected_tab = $(this).attr('id');
			if (gBody2.all_files_selected_tab == "workroom_files"){
				gBody2.get_channel_all_files(1);
				
			}else if (gBody2.all_files_selected_tab == "chatroom_files"){
				gBody2.get_chat_all_files(1);
				
			}else if (gBody2.all_files_selected_tab == "all_files"){
				gBody2.get_all_files(1);
			}
		});
		...
		$layer.find('#file_transfer').off().on('click', function(){	
			...
			$("input[name=file_checkbox]:checked").each(function() {
				...
				if (gBody2.all_files_selected_tab != "chatroom_files"){
				...
				if (ty == "2"){	
					if (file_check2.length > 1){
						gBody.send_files_chat("6", file_check2, file_md52, file_size2, file_name2, gBody2.all_files_selected_member, file_path2);
					}else{
						gBody.send_files_chat("6", file_check[0], file_md5[0], file_size[0], file_name[0], gBody2.all_files_selected_member, file_path[0]);
					}				
				}else{
					gBody3.send_files(ty, file_check, file_md5, file_size, file_name, gBody2.all_files_selected_member);
				}	
...
//			}			
		...
		$.ajax({
			...
			success : function(__res){
				...
				if (res.result == "OK"){
					...
					gap.total_page_count = gBody2._getPageCount(gap.total_data_count, gap.per_page);
					gBody2._initializePage();
	...
	"_initializeNavigator" : function(){
		...
		if (alldocuments != 0) {
			...
			if (c_frame == 1){
				...
				nav[0] = '<div class="arrow prev" onclick="gBody2._gotoPage(' + ((((c_frame-1) * 10) - 1)*gap.per_page + 1) + ',' + ((c_frame - 1) * 10) + ');"></div><ul class="pagination inb">';
			}			
			...
			for (var i = start_page; i < start_page + 10; i++){
				if (i == gap.cur_page){
					...
					if (i == '1'){
						nav[pIndex] = '<li onclick="gBody2._gotoPage(' + (((i-1) * gap.per_page) + 1 ) + ', ' + i + ', this)">' + i + '</li>';
					}else{
						if (i % 10 == '1' ){
							nav[pIndex] = '<li onclick="gBody2._gotoPage(' + (((i-1) * gap.per_page) + 1 ) + ', ' + i + ', this)">' + i + '</li>';
						}else{
			...
			if (c_frame < all_frame){
				nav[nav.length] = '</ul><div class="arrow next" onclick="gBody2._gotoPage(' + ((c_frame * gap.per_page * 10) + 1) + ',' + ((c_frame * 10) + 1) + ');"></div>';
			...
			if (c_frame != 1 ){
			//	nav_html = '<li class="p-first" onclick="gBody2._gotoPage(1,1);"><span>처음</span></li>';
			}		    
			for( var i = 0 ; i < nav.length ; i++){	
				nav_html += nav[i];
			}
			...
			if (c_frame < all_frame){
			//	nav_html += '<li class="p-last" onclick="gBody2._gotoPage(' + ((gBody2.all_page - 1) * gap.per_page + 1) + ',' + gBody2.all_page + ')"><span>마지막</span></li>';
			}
	...
	"_gotoPage" : function(idx, page_num){
		...
		if (gBody2.all_files_selected_tab == "workroom_files"){
			gBody2.get_channel_all_files(cur_page);
			
		}else if (gBody2.all_files_selected_tab == "chatroom_files"){
			gBody2.get_chat_all_files(cur_page);
			
		}else if (gBody2.all_files_selected_tab == "all_files"){
			gBody2.get_all_files(cur_page);			
		}
	},
	...
	"update_display_members" : function(ty, id, new_owner){
		...
		if (ty == "1"){
			...
			var postData = {
					"ch_code" : id,
					"ch_name" : gBody2.select_drive_name,
					"owner" : new_owner,
					"readers" : readers,
					"member" : user_list
				};
	...
	"draw_user_schedule" : function(list){
		...
		gBody2.quick_cal = $('#user_schedule').mobiscroll().eventcalendar({
			...
			onInit: function(event, inst){
				...
				gBody2.getQuickUserEvent(s_key, e_key, members_info, members_ky_list);
			},
			...
			onPageChange: function(event, inst){
				...
				var s_key = moment(first_day).format('YYYYMMDD[T000000Z]');
				var e_key = moment(last_day).format('YYYYMMDD[T235959Z]');
				var evt = gBody2.quick_cal.getEvents(s_key, e_key);

				var members_info = [];
				var members_ky = [];
				var members_ky_list = ""
				var prev_rsc_list = gBody2.quick_cal._resourcesMap;
				...
				$(evt).each(function(idx, val){
					gBody2.quick_cal.removeEvent(val.id);
				});
				gBody2.getQuickUserEvent(s_key, e_key, members_info, members_ky_list);	
			},
			dragToCreate: false,
		...
		if (gBody2.quick_cal){
			...
			$.ajax({
				...
				success : function(__res){
					...
					gBody2.quick_cal.setOptions({
						resources: rsc_list
					});
					
					var first_day = gBody2.quick_cal._firstDay;
					var last_day = gBody2.quick_cal._lastDay;
					var s_key = moment(first_day).format('YYYYMMDD[T000000Z]');
					var e_key = moment(last_day).format('YYYYMMDD[T235959Z]');
					
					gBody2.getQuickUserEvent(s_key, e_key, members_info, info.ky);
				},
				error : function(e){
					gap.gAlert(gap.lang.errormsg);
					return false;
	...
	"getQuickUserEvent" : function(s_key, e_key, members_info, members_ky_list){
		for (var i = 0; i < members_info.length; i++){
			...
			gBody2.setQuickCalEvent(url, mf, ky);
		}
		gBody2.setQuickUserStatus(members_ky_list);	
	},
		...
		$.ajax({
			...
			success: function(res){
				...
				$.each(res.viewentry, function(idx, val){
					...
					var evt = gBody2.getCalEventJson(val, is_my_event);
					if (!evt) return true;
				...
				if (gHome.getMainType() == '1') {
					gBody2.quick_cal.addEvent(event_type_1);
					
				} else {
					gBody2.quick_cal.addEvent(event_type_2);
				}
	...
	"setQuickUserStatus" : function(members_ky_list){
		...
		var url = location.protocol + "//" + location.host + "/" + opath + "/cal/calendar.nsf/(ag_get_vaca_info)?OpenAgent&" + $.param(param);
		$.ajax({
			url: url,
			dataType: 'json',
...
/*							var s_key = "";
							...
							gBody2.quick_cal.addEvent(event);
						}
					}
				}
			}
	...
	"showEventPreview" : function(event, is_main){
		...
		var url = location.protocol + "//" + location.host + "/" + opath + "/cal/calendar.nsf/(ag_schedule_preview)?OpenAgent&" + $.param(data);
		$.ajax({
			url : url,
			success: function(data){
