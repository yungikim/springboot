...
gPortlet.prototype = {
			...
			$.when.apply($, content_arr).done(function() {
				...
				$("#btn_weather_refresh").on("click", function(){
					...
					gport.getCurrentPosition().then(function(data){
						...
						return gport.weather_data_load(data, "", gport.weather_retry_count);
					});
				});
					...
					$(".category_approval").on("click", function(){
						...
						$.ajax({
							...
							success : function(res){
								...
								//해당 카테고리의 결재문서가 없을 때
								if(res.length === 0){
									var html2 = "<div class='null_doc'><span>해당 결재문서가 없습니다.</span></div>";
	...
	//날씨 데이터 불러오는 함수
	"weather_data_load": function(coord_data, flag, retryCount){
		//// retryCount 호출 재시도할 횟수 (3번까지)
		...
		return new Promise(function(resolve, reject) {
	        ...
	        var timeoutId = setTimeout(function () {
	            console.log("10초 초과! 날씨 API 재호출");
	            if (retryCount > 1) {
					gport.weather_retry_count = gport.weather_retry_count - 1;
	                gport.weather_data_load(coord_data, "", gport.weather_retry_count); // 재시도 횟수 감소 후 호출 재시도
	            } else {
					gport.weather_retry_count = 3;
				}
	        }, 5000); // 5초 초과시 호출 재시도
			...
			$.ajax({
	            ...
	            success: function(data){
					...
					clearTimeout(timeoutId); // 응답이 왔으므로 타이머 해제
					...
					$("#btn_weather_refresh").on("click", function(){
						...
						gport.getCurrentPosition().then(function(data){	
							...
							return gport.weather_data_load(coord_data, "", gport.weather_retry_count);
	            ...
	            error: function(xhr, error){
					clearTimeout(timeoutId); // 오류 발생 시 타이머 해제
		...
		return new Promise(function(resolve, reject) {
			...
			$.ajax({
	            ...
	            success: function(data){
					...
					item += "				<li class='category category_board board_notice active' id='tab_bbs_notice'><span class='category_name'>" + gap.lang.notice2 + "</span></li>";
					item += "				<li class='category category_board board_square' id='tab_bbs_square'><span class='category_name'>" + gap.lang.square + "</span></li>";
					item += "				<li class='category category_board board_cnc' id='tab_bbs_cnc'><span class='category_name'>" + gap.lang.cnc + "</span></li>";
					item += "				<li class='category category_board board_tip' id='tab_bbs_tip'><span class='category_name'>" + gap.lang.tipntech + "</span></li>";
					item += "			</div>";
							...
							if(key === 'item-weather'){
								...
								if($("[gs-id=item-weather]").length === 0){
									...
									gport.getCurrentPosition().then(function(data){
										...
										// data = 위치좌표, 좌표에 해당하는 장소 데이터
										return gport.weather_data_load(data, "add", gport.weather_retry_count);
									});
