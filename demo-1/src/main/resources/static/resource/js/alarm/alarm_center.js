function gAlarmCenter() {
	...
	/*** 미처리 건 수 시스템 표시에 사용할 변수 ***/
	this.total_sys_cnt = 0;
	this.complete_sys_cnt = 0;
	/*** 미처리 건 수 시스템 표시에 사용할 변수 ***/
...
gAlarmCenter.prototype = {
	...
	"init" : function(){
		...
		if(!is_mobile){
			...
			$(document).keydown(function(e) {
			    // Enter 키(13번 키)를 누른 경우
			    if (e.key === "Enter" || e.keyCode === 13) {
			        // 현재 포커스된 요소가 특정 input인지 확인
			        if (!$("#input_alarm_search").is(":focus")) {
			...
			/*********************** 모바일 ***************************/
			/// 모바일에서는 미처리 건 수 표시하지 않는다.
			gAct.draw_system_menu_list("mobile");
			/// 알림 미확인 건 수, 알림 이력보기 버튼, 모두 읽음 버튼 그리는 함수 //
					...
					if (evt == "noti"){
								...
								if (res == "OK"){
									...
									if (nid == "kp_channel"){
										...
										if (per_key == ""){
											...
											var url = gap.rp + "/noti/alarm/confirm";
											$.ajax({
												type : "POST",
												url : url,
												xhrFields : {
												...
												success : function(res){
													if (res.result == "success"){
														gap.alarm_content_popup(nid, _evdata.rid, _evdata);
													}
												},
												error : function(e){
												}
											...
											var url = gap.rp + "/noti/alarm/list";
											$.ajax({
												type : "POST",
												url : url,
												xhrFields : {
										...
										}else if (nid == "calendar" && _evdata.data.tid == ""){
											// 일정 미리 알림인 경우
											gap.alarm_content_popup(nid, _evdata.rid, _evdata);
											return;
										}else{
						...
						if (nid == "kp_channel" || nid == "kp_chat" || nid == "kp_mail"){
							...
							if (nid == "kp_channel"){
								...
								url = gap.rp + "/noti/alarm/chat/opt-set";
								...
								url = gap.rp + "/" + maildbpath + "/process_action?OpenForm";								
							}
						...
						// 세션 갱신
						var sso_url = _evdata.url;
						setTimeout(function(){
						//	location.href = sso_url + location.protocol + '//' + location.host + cdbpath + "/main?readform";
							location.href = sso_url;
						}, 3000);						
						
						mobiscroll.toast({message:gap.lang.processing_session.replace("$", "\n"), cssClass:"custom-toast", color:'danger'});
						return false;						
					}
				});
			}
	...
	"confirm_app_event_data" : function(_evdata){
		...
		if ($("#tab_unread_list").hasClass("select")){
			...
			if (fix_el_disp == "none" || fix_el_disp == undefined){
				// 접혀 있는 경우
				var url = gap.rp + "/noti/alarm/group-sum";
				$.ajax({
					type : "GET",
					url : url,
					xhrFields : {
				...
				if (_evdata.data.ty == "nid"){
					...
					// rid로 처리하는 경우 ( ex. 채팅방(채팅방은 rid, id 둘 다 처리) )
			...
			if (detail_list_layer_disp == "none" || detail_list_layer_disp == undefined){
				...
				if (nid == layer_nid){
					// 이벤트로 들어온 nid와 열려 있는 레이어의 nid가 같은 경우
					$(_evdata.data.data).each(function(idx, val){
	...
	"draw_main_alarm_title" : function(){
		...
		html +=	"	<div id='alarm_center_title' class='title_wrap'>";
		if(!is_mobile){
			html += "		<div><img class='logo_img' src='" + root_path + "/resource/images/alarm_center/alarm_center_logo_new.png?open&ver=" + jsversion + "'></div>";
		}
		...
		///PC 클라이언트  APP
		if(!is_mobile){
			$("#alarm_center_title").off().on("click", function(){
				var check_url = root_path + "/kwa01/sso.nsf/check.txt?open&ver="+new Date().getTime();
				$.ajax({
					type : "GET",
					url : check_url,
		...
		$("#btn_alarm_minimize").on("mouseenter", function(){
			$(this).addClass("active");
		});
		$("#btn_alarm_minimize").on("mouseleave", function(){
			$(this).removeClass("active");
		});
		...
		$("#btn_alarm_close").off().on("click", function(){
			if($("#alarm_setting_box_wrap").length !== 0){
				...
				if (is_mobile){
					...
					if (window.chrome.webview){
						window.chrome.webview.postMessage('{"type":"action", "stype":"close"}');				
					}					
				}
			}
		});
											...
											success : function(res){
												if (res == "OK"){
													if (info.noti_id == "kp_mail"){
														url = gap.rp + "/" + mailfile + "/FrameMail?openform";
														if (window.chrome.webview){
														...
														if (url != "" && url != "-none-"){
															...
															if (url.indexOf("loginid") > -1){
																url = url.replace("loginid", gap.userinfo.rinfo.id);
															}
															if (url.indexOf("ou1") > -1){
																if (info.noti_id == "drmcount"){
																	// drm은 사번을 소문자로만...
																	url = url.replace("ou1", gap.userinfo.rinfo.ky.toLowerCase());
																}else{
																	url = url.replace("ou1", gap.userinfo.rinfo.ky);
																}
															}
															if (url.indexOf("ou6") > -1){
																url = url.replace("ou6", gap.userinfo.rinfo.emp);
															}
															if (url.indexOf("comcode") > -1){
																url = url.replace("comcode", gap.userinfo.rinfo.cpc);
															}
	...
	"search_unread_count" : function(item_list){
		//// 사용자가 활성화한 총 시스템 갯수를 변수에 담는다. 
		gAct.total_sys_cnt = item_list.length;
		
		/// show_unprocessed_sys에서 더해지는 변수 0으로 초기화
		gAct.complete_sys_cnt = 0;
	...
	"search_sys_unread_count" : function(item){
		...
		if (code == "kp_mail"){
			ckurl = gap.rp + "/"+maildbpath+"/XML_Inbox_unread?ReadViewEntries&outputformat=json&start=1&count=1&charset=utf-8&"+new Date().getTime();
		}
		if (ckurl != ""){
			if (ckurl.slice(-3) == ".km" || ckurl.slice(-3) == ".ch"){
				...
				if (ckurl != "-none-"){
					...
					gAct.show_unprocessed_sys(0, code);
			...
			if (code == "kp_mail"){
				$.ajax({
					...
					success : function(res){						
						...
						gAct.show_unprocessed_sys(unread_count, code);
						
					},
					error : function(e){
						
						/// 호출 실패에도 호출해야 함수를 탈때마다 complete_sys_cnt 카운트를 올릴 수 있음
						gAct.show_unprocessed_sys(0, code);
	...
	"show_unprocessed_sys" : function(count, code){
		...
		if (count > 0){
			//// 999건 초과일 때 999+로 표시
			if(count > 999){
				$("#unprocessed_sys_" + code).find(".count_wrap").html("999+");
			}
			$("#empty_unprocessed").remove();
			$("#unprocessed_sys_" + code).show();
			
			$("#unprocessed_sys_" + code).addClass("show_sys");	
		} else{
			$("#unprocessed_sys_" + code).hide();
			$("#unprocessed_sys_" + code).removeClass("show_sys");
		}
		
		gAct.complete_sys_cnt ++;
		
		//// 보여질 시스템 체크를 끝냈을 때
		if(gAct.total_sys_cnt <= gAct.complete_sys_cnt){
			
			//// 하나라도 보여질 시스템이 없다면 
			if( $("#system_menu_list .menu_li.show_sys").length === 0 ){
				
				/*** 미처리 건 수 없음 문구를 표시 ***/
				if( $("#empty_unprocessed").length === 0 ){
					var html = "";
					
					html += "<div id='empty_unprocessed' class='empty_unprocessed_box'>";
					html += "	<span class='unprocessed_ico'></span>";
					html += "	<span class='unprocessed_txt'>" + gap.lang.not_found_unprocessed + "</span>";
					html += "</div>";
					
					$("#system_menu_list").find(".menu_li").hide();
					$("#system_menu_list").append(html);
				}
				/*** 보여질 시스템이 없다는 문구를 표시 ***/
			}
	...
	"change_rp_server" : function(ckurl){
		if (gap.isDev){
			ckurl = ckurl.replace("http://dapp2.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://dapp1.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://dst.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://mdmdev.daesang.com:7777", gap.rp);
			ckurl = ckurl.replace("http://mdm0.daesang.com:8686", gap.rp);
			ckurl = ckurl.replace("http://192.168.14.152:8801", gap.rp);
			ckurl = ckurl.replace("http://devplm.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://192.168.14.121:8503", gap.rp);
			ckurl = ckurl.replace("http://192.168.9.19:8080", gap.rp);
			ckurl = ckurl.replace("http://192.168.15.250:8080", gap.rp + "/count01");
			ckurl = ckurl.replace("http://192.168.14.153:8080", gap.rp + "/count02");
		}else{
			ckurl = ckurl.replace("http://dspsec.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://dsp.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://dst.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://plm.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://vocweb.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://dsmdm.daesang.com", gap.rp);
			ckurl = ckurl.replace("http://mdm0.daesang.com:8686", gap.rp);
			ckurl = ckurl.replace("http://sangsang.daesang.com:9000", gap.rp);
			ckurl = ckurl.replace("http://dsdrm.daesang.com:8080", gap.rp + "/count02");
			ckurl = ckurl.replace("http://dsin.daesang.com", gap.rp +"/dsin");
			ckurl = ckurl.replace("http://192.168.15.138:8081", gap.rp + "/count01");
			ckurl = ckurl.replace("http://dps.daesang.com", gap.rp +"/count03");
		}
...
//		html += "			<button type='button' id='app_test' style='width:auto; margin-right: 8px;' class='btn_all_read_alarm'><span>앱 통신 테스트</span></button>";
	...
	"main_alarm_scroll_evt": function(){
		$("#alarm_box_wrap").find('.alarm_box').off().on('scroll', function () {
	      ...
	      //스크롤 최하단일 때 그라디언트 숨김
	        if (scrollTop + innerHeight + 12 >= scrollHeight) {
	...
	"draw_unread_alarm_group_one" : function(item, is_group_sum){
		...
		//// 99건 초과일 때 99+로 표시
			...
			html += "				<div class='alarm_desc_txt'>" + gap.message_check_alarm_center(item.last.msg) + "</div>";
			html += "			</div>";
			html += "		</div>";
			...
			if ( group_title_el.length == 0 ){
				...
				group_title_el.find(".alarm_desc_txt").html( gap.message_check_alarm_center(item.last.msg) );
				
				group_el.prependTo(group_el.parent());
			}
			
			gAct.main_alarm_scroll_evt();
					...
					success : function(res){
						if (res == "OK"){
							...
							$.ajax({
								...
								success : function(res){
									if (res.result == "success"){
										gap.alarm_content_popup(nid, rid, info);
	...
	"draw_unread_alarm_list_one" : function(item, target, nid, i, is_list){
		...
		html += "			</div>";
		html += "		</div>";
		html += "		<div class='unread_alarm_desc'>" + gap.message_check_alarm_center(item.msg) + "</div>";
		html += "	</div>";
		html += "</div>";
	...
	"show_user_profile": function(el){
		...
		if (is_mobile){
			if (_key != ""){
				var url_link = "kPortalMeet://NativeCall/callUserProfile?ky=" + encodeURIComponent(_key);
				gap.connectApp(url_link);
				return false;
			}
		}else{
		...
		$.ajax({
			...
			success : function(res){
				if (res == "OK"){
					...
					var url = gap.rp + "/noti/alarm/count-sum";
					$.ajax({
						type : "GET",
						url : url,
						xhrFields : {
						...
						$.ajax({
							...
							success : function(res){
								...
								if( observer === undefined && start === 0 ){
									/// 건 수는 처음에만 표시한다. (새 알림이 왔을 때도 업데이트)
								...
								for( var i = 0; i < detail_alarm_json.length; i++){
									...
									html += "				</div>";
									html += "			</div>";
									html += "			<div class='detail_alarm_desc'>" + gap.message_check_alarm_center(item.msg) + "</div>";
									html += "		</div>";
								...
								//if(qry === ""){
									...
									observer = new IntersectionObserver(entries => {
										...
										if (entries[0].isIntersecting) {
											...
											if( hasScroll ) {
												//console.log(">>>>>>>>>>>>>>> 스크롤을 통한 새 이력 데이터 요청");
												list_load("add");
											}
	...
	"detail_all_alarm_event_bind": function(type){
		...
		///알림 토글 버튼(종모양 아이콘)
		$("#detail_all_alarm .btn_alarm_toggle").off().on("click", function(e){
			e.stopPropagation();
	...
	"goHome" : function(){
		var url = location.protocol + "//" + (gap.isDev ? "dev" : "one") + ".kmslab.com";
		if (window.chrome.webview){
			window.chrome.webview.postMessage('{"type":"openurl", "stype":"' + url + '"}');									

		}else{
			gap.openOnce(url, 'dsw');
		}	
	},
	...
	"sessionCheck" : async function(){
		var check_url = root_path + "/kwa01/sso.nsf/check.txt?open&ver="+new Date().getTime();
		var res = await $.ajax({url: check_url});
		console.log(res);
	}
}