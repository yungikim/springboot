    ...
    this.each(function(){
      ...
      */
      function placeDropdown(eventType) {
        ...
        // 최대 높이를 지정함.
        if(activates.height() > 490){	
        	activates.css('height', '490');
        }
        ...
        }, 300);  //2019.03.28 크롬에서 메뉴가 바로 닫히는 문제가 있어서 setTimeout 시간을 0에서 300으로 늘림.
      }
  ...
  var methods = {
    init : function(options) {
      ...
      var animateIndicator = function(prev_index) {
          $indicator.css({"min-width": ''});
          $indicator.css({"max-width": ''}); 	  
        ...
        $indicator.css({"min-width": $this.find('.active').width()});
        $indicator.css({"max-width": $this.find('.active').width()}); 
...
;(function ($) {
  $(document).ready(function() {
    ...
    Materialize.updateTextFields = function() {
      var input_selector = 'input[type=text], input[type=password], input[type=email], input[type=url], input[type=tel], input[type=number], input[type=search], textarea:not([readonly])';
    ...
    function textareaAutoResize($textarea) {
      ...
      console.log(">>> hiddenDiv : " + hiddenDiv.height());
...
//    $('body').on('keyup keydown autoresize', text_area_selector, function () {
    $('body').on('keyup autoresize', text_area_selector, function () {
    ...
    $(this).each(function(){
      ...
      $newSelect.on({
        ...
        'mousedown': function (e){
          /**
           * IE dropdown close Bugfix ==> https://github.com/Dogfalo/materialize/issues/901
           * IE에서 스크롤 드래그시 드롭다운 메뉴 닫히는 버그 처리
           */
        	e.stopPropagation();
	        if (e.clientX >= e.target.clientWidth || e.clientY >= e.target.clientHeight) {
	        	if ($('ul.select-dropdown').not(e.target).not(options[0]).is(':visible')) {
	        		$('input.select-dropdown').trigger('close');
	        	}
	        	if (!options.is(':visible')) {
	        		$(this).trigger('open', ['focus']);
