		...
		"editor_build" : function(opt){
			var DEFAULT_OPTIONS = {
				...
				'imageUploadPath': function(type){
					var path = '';
					if (type != undefined && type == 'paste') {
						// 붙여넣기 이벤트시 호출되는 URL
						//path = '/common/images2.nsf/ImageUploadKEditor?OpenForm&ImageLinkKey=' + window._KEDITOR_ImageLinkKey + '&type=CONTROL';
						path = 'https://one.kmslab.com/kwa01/images2.nsf/ImageUploadKEditor?OpenForm&ImageLinkKey=' + window._KEDITOR_ImageLinkKey + '&type=CONTROL';
					} else {
						// 이미지 삽입 레이어에서 호출되는 URL
						path = '/' + window._KEDITOR_BASE_PATH + '/pages/ImageuploadPage.html?OpenForm&ver=3.0.2&ImageLinkKey=' + window._KEDITOR_ImageLinkKey;
					}
					return path
				},
				...
				'dropEvent':function(e){
					if(window.KAM || window.document.KAM){
						e.preventDefault();
						e.stopPropagation();
						if(__browser && __browser.is_firefox){ KAM.dropFile(e); } else { document.KAM.dropFile(e); }
					}
				},
				*/
				'imageLimitSize':(10 * 1024 * 1024), //10MB
	            'videoUploadPath': 'https://one.kmslab.com/kwa01/images2.nsf/VideoUploadKEditor?OpenForm',	//'/common/images2.nsf/VideoUploadKEditor?OpenForm',
	            'videoLimitSize':(10 * 1024 * 1024), //10MB
				'largeToolbar': true,
				'showBottomTab': true,
				'font_list':[
...
{ label: '<span f-style>宋体</span> (<span class="tx-txt">ABCDE</span>)', title: '宋体', data: 'SimSun', klass: '' }
                ...
                'kwhInstallPath': '/' + window._KEDITOR_BASE_PATH + '/kwebhelper', // KWebHelper 설치 페이지 경로
                'kwhVersion': (navigator.userAgent.toLowerCase().indexOf('mac')>-1?'1.0.0.18':'1.0.0.19'), // KWebHelper 버전
                'kwhAutoPopup': false, // 웹 에디터 로드 시 마다 KWebHelper 설치 확인 여부 (KWebHelper가 설치되어 있지 않은 경우 설치 페이지로 이동, 윈도우 OS만 동작됨, 모든 사용자가 KWH 이용)
                'kwhIcon': true, // KWebHelper 설치 아이콘 표시 여부 (아이콘 클릭 시 KWebHelper 설치 페이지로 이동, 윈도우 OS만 표시됨, 원하는 사용자만 KWH 이용)
				'core':{}
			};
			
			var options = $.extend({}, DEFAULT_OPTIONS, opt);
			return KEditor.build(options);
		},
		"_init_edit":function(){
			var _self = this;
			...
			/*
			_self.keditor = _self.editor_build({'instanceId':'001',	'wrapper':'#editor_wrapper'	});
			_self.keditor['do'](function(adapter) {
				_self.keditor2 = _self.editor_build({'instanceId':'002','wrapper':'#editor_wrapper2'});
				_self.keditor2['do'](function(adapter) {
					_self.keditor3 = _self.editor_build({'instanceId':'003','wrapper':'#editor_wrapper3'});
					_self.keditor3['do'](function(adapter) {
						_self.keditor4 = _self.editor_build({'instanceId':'004','wrapper':'#editor_wrapper4'});
						_self.keditor4['do'](function(adapter) {
							_self.keditor5 = _self.editor_build({'instanceId':'005','wrapper':'#editor_wrapper5'});
							_self.keditor5['do'](function(adapter) {
								_self.keditor6 = _self.editor_build({'instanceId':'006','wrapper':'#editor_wrapper6'});
								_self.keditor6['do'](function(adapter) {
		...
		"load_complete": function(adapter) {
			...
			var duration = window.endTime - window.startTime;
			//alert('로딩시간 : ' + (duration / 1000) + 's');
...
//			}
			...
			/*
			this.keditor2 = KEditor.build({
				'instanceId':'002',
				'wrapper':'#editor_wrapper2',
				'language':(window._i18n_editor&&window._i18n_editor.language||'ko'),
				'form':document.forms[0].name,
				'iconPath': '/' + window._KEDITOR_BASE_PATH + '/images/icon/editor',
				'decoPath': '/' + window._KEDITOR_BASE_PATH + '/images/deco/contents',
			...
			*/
			 ...
			 *	도미노로 문서 본문 가져와 처리하는 방법
				...
				// 편집 모드이면 바로 에디터 콘텐츠를 세팅한다. - 2025.04.10
				adapter.setHtml(parent.gBody3.editor_html);				
			}
		},
