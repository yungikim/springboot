...
if(window.KEditor == null){
    (function(){
         ...
         *  kwhVersion: (navigator.userAgent.toLowerCase().indexOf('mac')>-1?'1.0.0.18':'1.0.0.18'), // KWebHelper 버전 (설치파일과 버전이 상이한 경우 KWH 설치아이콘이 빨간색으로 변경됨)
        ...
        Adapter.prototype = {
            'setHtml':function(_html) {
                ...
                // XSS 보안 필터 적용
                _html = _html.replace(/ onerror=".*?"/gi, "")
                            .replace(/ onclick=".*?"/gi, "")
                            .replace(/ onchange=".*?"/gi, "")
            ...
            'getHtmlValue': function(args){
                ...
                // 한줄로 되어 있는 HTML을 도미도 서버에서 MIME 처리할 때 일정한 크기에 도달하면 개행처리하는 과정에서 개행에 걸리는 문자열이 깨질 수 있다
                // 이러한 것을 방지하기 위해 1줄로 받는 html source code를 모두 개행처리한다.
                _body = '<html><head><style>' + this.kEditor.getHeadStyle() + '</style></head><body>' + _body + '</body></html>';
                _body = _body.split(/\>[ ]?\</).join(">\n<");
                _body = _body.replace(/\n?<br>\n?/gi, '<br>');
                return _body
            },
            'getDom':function() {
        ...
        EDITOR.prototype = {
            'init':function() {
                ...
                if(_self.config.font_default){
                    ...
                    // 본문 스타일을 별도록 지정한 경우
                    if (_self.config.bodyStyles) {
                        config.canvas.styles = $.extend(true, config.canvas.styles, _self.config.bodyStyles);
                    }
                }
                ...
                _self._editor = dfd.promise().then(
                    function(_editor) {
						clearInterval(_editor.waitEvent);
						//console.log(_self.instanceId);
                        ...
                        if (window.ImageUploader && (_self.config.kwhAutoPopup == true || _self.config.kwhIcon == true)) window.ImageUploader.deploy(_self); // KWebHelper initialize
            ...
            '_getUploadImageInfo':function(name, type, ext, upload_type) {
                ...
                var _url = (typeof this.config.imageUploadPath != 'string'?this.config.imageUploadPath('paste'):this.config.imageUploadPath)
				    //+ '&prefix=' +''
                    //+ '&type=CONTROL'
				    ;
                //var _host = document.location.protocol + "//" + document.location.host;
                /*
                로컬서버에서 웹 에디터 구동 시 _host 변수 null 처리 (이미지 생성 후 서버에서 host+url 포함하여 리턴)
                */ 
                var _host;
                if (document.location.hostname && document.location.hostname == 'local.k-portal.co.kr') {
                    _host = '';
                } else {
                    _host = document.location.protocol + "//" + document.location.host;
                }
            ...
            '_getUploadVideoInfo':function(ext) {
                /**
                 * 동영상을 서버에 저장할 때 필요한 정보 반환.
                 * @param name : 동영상 파일명
                 * @param type : 동영상 mime type
                 * @param ext : 동영상 확장자
                 * @return
                 *  {
                 *      cid: // 동영상 cid,
                 ...
                 *      url: // 동영상 Upload URL,
                 *          "blob"의 응답은 {"fileName":..., "url":...}
                 *      host: // 응답으로 받은 image 경로의 host ( host + 동영상 경로로 구성),
                 *      postdata:{
                 *          [name]:value
                 *      }
                 *      type:'domino'(default)...
                 *  }
                 */
                var _cid = (new Date).getTime() + '' + (this.videoCount++);
                var _name = "video." + _cid + '.' + ext;
                var _url = this.config.videoUploadPath;
                ...
                */ 
                ...
                if( _url.indexOf(_host) == -1) _url = _host + _url;
                return {
                    'type':'domino',
                    'cid':_cid,
                    'name':_name,
                    ...
                    if(uploadResult){
                        ...
                        if(json != null){
                            for(var i = 0; i < json.length; i++){
                                ...
                                if(imgs != null){
                                    for(var j = 0; j < imgs.length; j++){
                                        info.uploadedImage[json[i].fileName] = this._createImageUrl(info.host, json[i].url);
                                    }
                                }
                            } 
                        }
